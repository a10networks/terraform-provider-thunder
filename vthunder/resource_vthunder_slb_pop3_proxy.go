...
//vThunder resource SlbPop3Proxy
...
func resourceSlbPop3ProxyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		d.SetId("1")
		go_vthunder.PostSlbPop3Proxy(client.Token, data, client.Host)
...
func resourceSlbPop3ProxyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetSlbPop3Proxy(client.Token, client.Host)
		if data == nil {
...
func dataToSlbPop3Proxy(d *schema.ResourceData) go_vthunder.Pop3Proxy {
	var vc go_vthunder.Pop3Proxy
	var c go_vthunder.Pop3ProxyInstance

	SamplingEnableCount := d.Get("sampling_enable.#").(int)
	c.Counters1 = make([]go_vthunder.SamplingEnable28, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj1 go_vthunder.SamplingEnable28
