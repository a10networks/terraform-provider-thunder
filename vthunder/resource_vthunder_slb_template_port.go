...
func resourceTemplatePortRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetTemplatePort(client.Token, name, client.Host)
		if data == nil {
...
func resourceTemplatePortDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_vthunder.DeleteTemplatePort(client.Token, name, client.Host)
		if err != nil {
...
func dataToTemplatePort(d *schema.ResourceData) go_vthunder.Port {
	var vc go_vthunder.Port
	var c go_vthunder.PortInstance
	c.Add = d.Get("add").(int)
