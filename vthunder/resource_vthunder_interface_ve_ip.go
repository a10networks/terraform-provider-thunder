...
//vThunder resource InterfaceVeIP
...
func resourceInterfaceVeIPCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		go_vthunder.PostInterfaceVeIP(client.Token, name, data, client.Host)
...
func resourceInterfaceVeIPRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetInterfaceVeIP(client.Token, name, client.Host)
		if data == nil {
...
func dataToInterfaceVeIP(d *schema.ResourceData) go_vthunder.VeIP {
	var vc go_vthunder.VeIP
	var c go_vthunder.VeIPInstance
	c.GenerateMembershipQuery = d.Get("generate_membership_query").(int)

	AddressListCount := d.Get("address_list.#").(int)
	c.Ipv4Address = make([]go_vthunder.VeIPAddressList, 0, AddressListCount)

	for i := 0; i < AddressListCount; i++ {
		var obj1 go_vthunder.VeIPAddressList
	...
	c.HelperAddress = make([]go_vthunder.VeIPHelperAddressList, 0, HelperAddressListCount)

	for i := 0; i < HelperAddressListCount; i++ {
		var obj2 go_vthunder.VeIPHelperAddressList
	...
	var obj3 go_vthunder.VeIPStatefulFirewall
	...
	var obj4 go_vthunder.VeIPRip
	prefix = "rip.0."

	var obj4_1 go_vthunder.VeIPReceiveCfg
	...
	var obj4_4 go_vthunder.VeIPSendCfg
	...
	var obj5 go_vthunder.VeIPRouter
	prefix = "router.0."

	var obj5_1 go_vthunder.VeIPIsis
	...
	var obj6 go_vthunder.VeIPOspf
	prefix = "ospf.0."

	OspfIpListCount := d.Get(prefix + "ospf_ip_list.#").(int)
	obj6.DeadInterval = make([]go_vthunder.VeIPOspfIPList, 0, OspfIpListCount)

	for i := 0; i < OspfIpListCount; i++ {
		var obj6_1 go_vthunder.VeIPOspfIPList
		...
		obj6_1.MessageDigestKey = make([]go_vthunder.VeIPMessageDigestCfg, 0, MessageDigestCfgCount)

		for i := 0; i < MessageDigestCfgCount; i++ {
			var obj6_1_1 go_vthunder.VeIPMessageDigestCfg
			prefix2 = prefix1 + fmt.Sprintf("message_digest_cfg.%d.", i)
			obj6_1_1.MessageDigestKey = d.Get(prefix2 + "message_digest_key").(int)

			var obj6_1_1_1 go_vthunder.VeIPMd5
	...
	var obj6_2 go_vthunder.VeIPOspfGlobal
	...
	var obj6_2_1 go_vthunder.VeIPNetwork
	...
	var obj6_2_2 go_vthunder.VeIPAuthenticationCfg
	...
	var obj6_2_3 go_vthunder.VeIPBfdCfg
	...
	var obj6_2_4 go_vthunder.VeIPDatabaseFilterCfg
	...
	MessageDigestCfgCount := d.Get(prefix1 + "message_digest_cfg.#").(int)
	obj6_2.MessageDigestKey = make([]go_vthunder.VeIPMessageDigestCfg, 0, MessageDigestCfgCount)

	for i := 0; i < MessageDigestCfgCount; i++ {
		var obj5 go_vthunder.VeIPMessageDigestCfg
		prefix2 = prefix1 + fmt.Sprintf("message_digest_cfg.%d.", i)
		obj5.MessageDigestKey = d.Get(prefix2 + "message_digest_key").(int)

		var obj1 go_vthunder.VeIPMd5
		prefix3 := prefix2 + "md5.0."
