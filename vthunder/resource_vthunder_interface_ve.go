...
func resourceInterfaceVECreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		go_vthunder.PostInterfaceVE(client.Token, data, client.Host)
...
func resourceInterfaceVERead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetInterfaceVE(client.Token, name, client.Host)
		if data == nil {
...
func resourceInterfaceVEUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received V from method data to InterfaceVE ")
		d.SetId(strconv.Itoa(name))
		go_vthunder.PutInterfaceVE(client.Token, strconv.Itoa(name), data, client.Host)
...
func resourceInterfaceVEDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_vthunder.DeleteInterfaceVE(client.Token, name, client.Host)
		if err != nil {
...
func dataToInterfaceVE(d *schema.ResourceData) go_vthunder.InterfaceVE {
	var vc go_vthunder.InterfaceVE
	var c go_vthunder.InterfaceVEInstance
	var obj1 go_vthunder.VeMap
	prefix := "map.0."
	...
	var obj2 go_vthunder.VeNptv6
	prefix = "nptv6.0."

	DomainListCount := d.Get(prefix + "domain_list.#").(int)
	obj2.DomainName = make([]go_vthunder.VeDomainList, 0, DomainListCount)

	for i := 0; i < DomainListCount; i++ {
		var obj2_1 go_vthunder.VeDomainList
	...
	var obj3 go_vthunder.VeIsis
	prefix = "isis.0."

	PriorityListCount := d.Get(prefix + "priority_list.#").(int)
	obj3.Priority = make([]go_vthunder.VePriorityList, 0, PriorityListCount)

	for i := 0; i < PriorityListCount; i++ {
		var obj3_1 go_vthunder.VePriorityList
	...
	obj3.HelloIntervalMinimal = make([]go_vthunder.VeHelloIntervalMinimalList, 0, HelloIntervalMinimalListCount)

	for i := 0; i < HelloIntervalMinimalListCount; i++ {
		var obj3_2 go_vthunder.VeHelloIntervalMinimalList
	...
	var obj3_3 go_vthunder.VeMeshGroup
	...
	var obj3_4 go_vthunder.VeAuthentication
	prefix1 = prefix + "authentication.0."

	SendOnlyListCount := d.Get(prefix1 + "send_only_list.#").(int)
	obj3_4.SendOnly = make([]go_vthunder.VeSendOnlyList, 0, SendOnlyListCount)

	for i := 0; i < SendOnlyListCount; i++ {
		var obj3_4_1 go_vthunder.VeSendOnlyList
	...
	obj3_4.Mode = make([]go_vthunder.VeModeList, 0, ModeListCount)

	for i := 0; i < ModeListCount; i++ {
		var obj3_4_2 go_vthunder.VeModeList
	...
	obj3_4.KeyChain = make([]go_vthunder.VeKeyChainList, 0, KeyChainListCount)

	for i := 0; i < KeyChainListCount; i++ {
		var obj3_4_3 go_vthunder.VeKeyChainList
	...
	obj3.CsnpInterval = make([]go_vthunder.VeCsnpIntervalList, 0, CsnpIntervalListCount)

	for i := 0; i < CsnpIntervalListCount; i++ {
		var obj3_5 go_vthunder.VeCsnpIntervalList
	...
	obj3.Password = make([]go_vthunder.VePasswordList, 0, PasswordListCount)

	for i := 0; i < PasswordListCount; i++ {
		var obj3_6 go_vthunder.VePasswordList
	...
	var obj3_7 go_vthunder.VeBfdCfg
	...
	obj3.WideMetric = make([]go_vthunder.VeWideMetricList, 0, WideMetricListCount)

	for i := 0; i < WideMetricListCount; i++ {
		var obj3_8 go_vthunder.VeWideMetricList
	...
	obj3.HelloInterval = make([]go_vthunder.VeHelloIntervalList, 0, HelloIntervalListCount)

	for i := 0; i < HelloIntervalListCount; i++ {
		var obj3_9 go_vthunder.VeHelloIntervalList
	...
	obj3.HelloMultiplier = make([]go_vthunder.VeHelloMultiplierList, 0, HelloMultiplierListCount)

	for i := 0; i < HelloMultiplierListCount; i++ {
		var obj3_10 go_vthunder.VeHelloMultiplierList
	...
	obj3.Metric = make([]go_vthunder.VeMetricList, 0, MetricListCount)

	for i := 0; i < MetricListCount; i++ {
		var obj3_11 go_vthunder.VeMetricList
	...
	var obj4 go_vthunder.VeBfd
	prefix = "bfd.0."

	var obj4_1 go_vthunder.VeIntervalCfg
	...
	var obj4_2 go_vthunder.VeAuthentication1
	prefix1 = prefix + "authentication.0."
	...
	obj5.HelperAddress = make([]go_vthunder.VeHelperAddressList, 0, HelperAddressListCount)

	for i := 0; i < HelperAddressListCount; i++ {
		var obj5_2 go_vthunder.VeHelperAddressList
	...
	var obj5_3 go_vthunder.VeStatefulFirewall
	...
	var obj5_4 go_vthunder.VeRip
	prefix1 = prefix + "rip.0."

	var obj5_4_1 go_vthunder.VeReceiveCfg
	...
	var obj5_4_4 go_vthunder.VeSendCfg
	...
	var obj5_5 go_vthunder.VeRouter
	prefix1 = prefix + "router.0."

	var obj5_5_1 go_vthunder.VeIsis1
	...
	var obj5_6 go_vthunder.VeOspf
	prefix1 = prefix + "ospf.0."

	OspfIpListCount := d.Get(prefix1 + "ospf_ip_list.#").(int)
	obj5_6.DeadInterval = make([]go_vthunder.VeOspfIPList, 0, OspfIpListCount)

	for i := 0; i < OspfIpListCount; i++ {
		var obj5_6_1 go_vthunder.VeOspfIPList
		...
		obj5_6_1.MessageDigestKey = make([]go_vthunder.VeMessageDigestCfg, 0, MessageDigestCfgCount)

		for i := 0; i < MessageDigestCfgCount; i++ {
			var obj5_6_1_1 go_vthunder.VeMessageDigestCfg
			prefix3 = prefix2 + fmt.Sprintf("message_digest_cfg.%d.", i)
			obj5_6_1_1.MessageDigestKey = d.Get(prefix3 + "message_digest_key").(int)

			var obj5_6_1_1_1 go_vthunder.VeMd5
	...
	var obj5_6_2 go_vthunder.VeOspfGlobal
	...
	var obj5_6_2_1 go_vthunder.VeNetwork
	...
	var obj5_6_2_2 go_vthunder.VeAuthenticationCfg
	...
	var obj5_6_2_3 go_vthunder.VeBfdCfg
	...
	var obj5_6_2_4 go_vthunder.VeDatabaseFilterCfg
	...
	MessageDigestCfgCount := d.Get(prefix2 + "message_digest_cfg.#").(int)
	obj5_6_2.MessageDigestKey = make([]go_vthunder.VeMessageDigestCfg, 0, MessageDigestCfgCount)

	for i := 0; i < MessageDigestCfgCount; i++ {
		var obj5_6_2_5 go_vthunder.VeMessageDigestCfg
		prefix3 = prefix2 + fmt.Sprintf("message_digest_cfg.%d.", i)
		obj5_6_2_5.MessageDigestKey = d.Get(prefix3 + "message_digest_key").(int)

		var obj5_6_2_5_1 go_vthunder.VeMd5
		prefix4 := prefix3 + "md5.0."
	...
	var obj6 go_vthunder.VeIcmpv6RateLimit
	...
	c.Counters1 = make([]go_vthunder.VeSamplingEnable, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj7 go_vthunder.VeSamplingEnable
		prefix = fmt.Sprintf("sampling_enable.%d.", i)
		obj7.Counters1 = d.Get(prefix + "counters1").(string)
		c.Counters1 = append(c.Counters1, obj7)
	}

	var obj8 go_vthunder.VeLw4O6
	prefix = "lw_4o6.0."
	obj8.Outside = d.Get(prefix + "outside").(int)
	obj8.Inside = d.Get(prefix + "inside").(int)
	c.Outside = obj8

	var obj9 go_vthunder.VeIpv61
	prefix = "ipv6.0."
	obj9.Inbound = d.Get(prefix + "inbound").(int)

	AddressListCount = d.Get(prefix + "address_list.#").(int)
	obj9.AddressType = make([]go_vthunder.VeAddressList, 0, AddressListCount)

	for i := 0; i < AddressListCount; i++ {
		var obj9_1 go_vthunder.VeAddressList
		prefix1 = prefix + fmt.Sprintf("address_list.%d.", i)
	...
	var obj9_2 go_vthunder.VeRip1
	prefix1 = prefix + "rip.0."

	var obj9_2_1 go_vthunder.VeSplitHorizonCfg
	prefix2 = prefix1 + "split_horizon_cfg.0."
	...
	var obj9_3 go_vthunder.VeStatefulFirewall1
	prefix1 = prefix + "stateful_firewall.0."
	...
	var obj9_4 go_vthunder.VeRouter1
	prefix1 = prefix + "router.0."

	var obj9_4_1 go_vthunder.VeRipng
	prefix2 = prefix1 + "ripng.0."
	obj9_4_1.Rip = d.Get(prefix2 + "rip").(int)
	obj9_4.Rip = obj9_4_1

	var obj9_4_2 go_vthunder.VeOspf1
	prefix2 = prefix1 + "ospf.0."

	AreaListCount := d.Get(prefix2 + "area_list.#").(int)
	obj9_4_2.AreaIDAddr = make([]go_vthunder.VeAreaList, 0, AreaListCount)

	for i := 0; i < AreaListCount; i++ {
		var obj9_4_2_1 go_vthunder.VeAreaList
	...
	var obj9_4_3 go_vthunder.VeIsis1
	prefix2 = prefix1 + "isis.0."
	...
	var obj9_5 go_vthunder.VeOspf2
	prefix1 = prefix + "ospf.0."
	obj9_5.Bfd = d.Get(prefix1 + "bfd").(int)

	CostCfgCount := d.Get(prefix1 + "cost_cfg.#").(int)
	obj9_5.Cost = make([]go_vthunder.VeCostCfg, 0, CostCfgCount)

	for i := 0; i < CostCfgCount; i++ {
		var obj9_5_1 go_vthunder.VeCostCfg
	...
	obj9_5.Priority = make([]go_vthunder.VePriorityCfg, 0, PriorityCfgCount)

	for i := 0; i < PriorityCfgCount; i++ {
		var obj9_5_2 go_vthunder.VePriorityCfg
	...
	obj9_5.HelloInterval = make([]go_vthunder.VeHelloIntervalCfg, 0, HelloIntervalCfgCount)

	for i := 0; i < HelloIntervalCfgCount; i++ {
		var obj9_5_3 go_vthunder.VeHelloIntervalCfg
	...
	obj9_5.MtuIgnore = make([]go_vthunder.VeMtuIgnoreCfg, 0, MtuIgnoreCfgCount)

	for i := 0; i < MtuIgnoreCfgCount; i++ {
		var obj9_5_4 go_vthunder.VeMtuIgnoreCfg
	...
	obj9_5.RetransmitInterval = make([]go_vthunder.VeRetransmitIntervalCfg, 0, RetransmitIntervalCfgCount)

	for i := 0; i < RetransmitIntervalCfgCount; i++ {
		var obj9_5_5 go_vthunder.VeRetransmitIntervalCfg
	...
	obj9_5.TransmitDelay = make([]go_vthunder.VeTransmitDelayCfg, 0, TransmitDelayCfgCount)

	for i := 0; i < TransmitDelayCfgCount; i++ {
		var obj9_5_6 go_vthunder.VeTransmitDelayCfg
	...
	obj9_5.NeighborPriority = make([]go_vthunder.VeNeighborCfg, 0, NeighborCfgCount)

	for i := 0; i < NeighborCfgCount; i++ {
		var obj9_5_7 go_vthunder.VeNeighborCfg
	...
	obj9_5.BroadcastType = make([]go_vthunder.VeNetworkList, 0, NetworkListCount)

	for i := 0; i < NetworkListCount; i++ {
		var obj9_5_8 go_vthunder.VeNetworkList
	...
	obj9_5.DeadInterval = make([]go_vthunder.VeDeadIntervalCfg, 0, DeadIntervalCfgCount)

	for i := 0; i < DeadIntervalCfgCount; i++ {
		var obj9_5_9 go_vthunder.VeDeadIntervalCfg
	...
	var obj9_6 go_vthunder.VeRouterAdver
	...
	obj9_6.NotAutonomous = make([]go_vthunder.VePrefixList, 0, PrefixListCount)

	for i := 0; i < PrefixListCount; i++ {
		var obj9_6_1 go_vthunder.VePrefixList
	...
	var obj10 go_vthunder.VeAccessList
	...
	var obj11 go_vthunder.VeIcmpRateLimit
