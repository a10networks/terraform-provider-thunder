...
//vThunder resource FwTapMonitor
...
func resourceFwTapMonitorCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		go_vthunder.PostFwTapMonitor(client.Token, data, client.Host)
...
func resourceFwTapMonitorRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetFwTapMonitor(client.Token, client.Host)
		if data == nil {
...
func dataToFwTapMonitor(d *schema.ResourceData) go_vthunder.FwTapMonitor {
	var vc go_vthunder.FwTapMonitor
	var c go_vthunder.FwTapMonitorInstance
	c.Status = d.Get("status").(string)

	TapPortCfgCount := d.Get("tap_port_cfg.#").(int)
	c.TapEth = make([]go_vthunder.FwTapMonitorPortCfg, 0, TapPortCfgCount)

	for i := 0; i < TapPortCfgCount; i++ {
		var obj1 go_vthunder.FwTapMonitorPortCfg
