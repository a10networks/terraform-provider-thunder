...
func resourceFwAlgPptpCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		go_vthunder.PostFwAlgPptp(client.Token, data, client.Host)
...
func resourceFwAlgPptpRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetFwAlgPptp(client.Token, client.Host)
		if data == nil {
...
func dataToFwAlgPptp(d *schema.ResourceData) go_vthunder.FwAlgPptp {
	var vc go_vthunder.FwAlgPptp
	var c go_vthunder.FwAlgPptpInstance
	c.DefaultPortDisable = d.Get("default_port_disable").(string)

	SamplingEnableCount := d.Get("sampling_enable.#").(int)
	c.Counters1 = make([]go_vthunder.FwAlgPptpSamplingEnable, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj1 go_vthunder.FwAlgPptpSamplingEnable
