...
//vThunder resource InterfaceVeIPv6
...
func resourceInterfaceVeIPv6Create(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		go_vthunder.PostInterfaceVeIPv6(client.Token, name, data, client.Host)
...
func resourceInterfaceVeIPv6Read(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetInterfaceVeIPv6(client.Token, name, client.Host)
		if data == nil {
...
func dataToInterfaceVeIPv6(d *schema.ResourceData) go_vthunder.VeIPv6 {
	var vc go_vthunder.VeIPv6
	var c go_vthunder.VeIPv6Instance
	c.Inbound = d.Get("inbound").(int)

	AddressListCount := d.Get("address_list.#").(int)
	c.AddressType = make([]go_vthunder.VeIPv6AddressList, 0, AddressListCount)

	for i := 0; i < AddressListCount; i++ {
		var obj1 go_vthunder.VeIPv6AddressList
	...
	var obj2 go_vthunder.VeIPv6Rip
	prefix := "rip.0."

	var obj2_1 go_vthunder.VeIPv6SplitHorizonCfg
	...
	var obj3 go_vthunder.VeIPv6StatefulFirewall
	...
	var obj4 go_vthunder.VeIPv6Router
	prefix = "router.0."

	var obj4_1 go_vthunder.VeIPv6Ripng
	prefix1 = prefix + "ripng.0."
	obj4_1.Rip = d.Get(prefix1 + "rip").(int)
	obj4.Rip = obj4_1

	var obj4_2 go_vthunder.VeIPv6Ospf1
	prefix1 = prefix + "ospf.0."

	AreaListCount := d.Get(prefix1 + "area_list.#").(int)
	obj4_2.AreaIDAddr = make([]go_vthunder.VeIPv6AreaList, 0, AreaListCount)

	for i := 0; i < AreaListCount; i++ {
		var obj4_2_1 go_vthunder.VeIPv6AreaList
	...
	var obj4_3 go_vthunder.VeIPv6Isis
	...
	var obj5 go_vthunder.VeIPv6Ospf
	prefix = "ospf.0."
	obj5.Bfd = d.Get(prefix + "bfd").(int)

	CostCfgCount := d.Get(prefix + "cost_cfg.#").(int)
	obj5.Cost = make([]go_vthunder.VeIPv6CostCfg, 0, CostCfgCount)

	for i := 0; i < CostCfgCount; i++ {
		var obj5_1 go_vthunder.VeIPv6CostCfg
	...
	obj5.Priority = make([]go_vthunder.VeIPv6PriorityCfg, 0, PriorityCfgCount)

	for i := 0; i < PriorityCfgCount; i++ {
		var obj5_2 go_vthunder.VeIPv6PriorityCfg
	...
	obj5.HelloInterval = make([]go_vthunder.VeIPv6HelloIntervalCfg, 0, HelloIntervalCfgCount)

	for i := 0; i < HelloIntervalCfgCount; i++ {
		var obj5_3 go_vthunder.VeIPv6HelloIntervalCfg
	...
	obj5.MtuIgnore = make([]go_vthunder.VeIPv6MtuIgnoreCfg, 0, MtuIgnoreCfgCount)

	for i := 0; i < MtuIgnoreCfgCount; i++ {
		var obj5_4 go_vthunder.VeIPv6MtuIgnoreCfg
	...
	obj5.RetransmitInterval = make([]go_vthunder.VeIPv6RetransmitIntervalCfg, 0, RetransmitIntervalCfgCount)

	for i := 0; i < RetransmitIntervalCfgCount; i++ {
		var obj5_5 go_vthunder.VeIPv6RetransmitIntervalCfg
	...
	obj5.TransmitDelay = make([]go_vthunder.VeIPv6TransmitDelayCfg, 0, TransmitDelayCfgCount)

	for i := 0; i < TransmitDelayCfgCount; i++ {
		var obj5_6 go_vthunder.VeIPv6TransmitDelayCfg
	...
	obj5.NeighborPriority = make([]go_vthunder.VeIPv6NeighborCfg, 0, NeighborCfgCount)

	for i := 0; i < NeighborCfgCount; i++ {
		var obj5_7 go_vthunder.VeIPv6NeighborCfg
	...
	obj5.BroadcastType = make([]go_vthunder.VeIPv6NetworkList, 0, NetworkListCount)

	for i := 0; i < NetworkListCount; i++ {
		var obj5_8 go_vthunder.VeIPv6NetworkList
	...
	obj5.DeadInterval = make([]go_vthunder.VeIPv6DeadIntervalCfg, 0, DeadIntervalCfgCount)

	for i := 0; i < DeadIntervalCfgCount; i++ {
		var obj5_9 go_vthunder.VeIPv6DeadIntervalCfg
	...
	var obj6 go_vthunder.VeIPv6RouterAdver
	...
	obj6.NotAutonomous = make([]go_vthunder.VeIPv6PrefixList, 0, PrefixListCount)

	for i := 0; i < PrefixListCount; i++ {
		var obj6_1 go_vthunder.VeIPv6PrefixList
