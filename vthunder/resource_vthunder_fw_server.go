...
func resourceFwServerRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetFwServer(client.Token, name, client.Host)
		if data == nil {
...
func resourceFwServerUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received formatted data from method data to FwServer ")
		d.SetId(name)
		go_vthunder.PutFwServer(client.Token, name, data, client.Host)
...
func resourceFwServerDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_vthunder.DeleteFwServer(client.Token, name, client.Host)
		if err != nil {
...
func dataToFwServer(d *schema.ResourceData) go_vthunder.FwServer {
	var vc go_vthunder.FwServer
	var c go_vthunder.FwServerInstance
	c.HealthCheckDisable = d.Get("health_check_disable").(int)

	PortListCount := d.Get("port_list.#").(int)
	c.Protocol = make([]go_vthunder.FwServerPortList, 0, PortListCount)

	for i := 0; i < PortListCount; i++ {
		var obj1 go_vthunder.FwServerPortList
		...
		obj1.Counters1 = make([]go_vthunder.FwServerSamplingEnable, 0, SamplingEnableCount)

		for i := 0; i < SamplingEnableCount; i++ {
			var obj1_1 go_vthunder.FwServerSamplingEnable
	...
	c.Counters1 = make([]go_vthunder.FwServerSamplingEnable, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj1 go_vthunder.FwServerSamplingEnable
