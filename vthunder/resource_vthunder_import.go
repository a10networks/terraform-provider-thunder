...
func resourceImportCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		vc := dataToImport(d)
		d.SetId(name)
		go_vthunder.PostImport(client.Token, vc, client.Host)
...
func resourceImportRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		vc, err := go_vthunder.GetImport(client.Token, client.Host)
...
func dataToImport(d *schema.ResourceData) go_vthunder.Import {
	var vc go_vthunder.Import

	var c go_vthunder.ImportInstance
