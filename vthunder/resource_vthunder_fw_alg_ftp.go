...
func dataToFwAlgFtp(d *schema.ResourceData) go_vthunder.FwAlgFtp {
	var vc go_vthunder.FwAlgFtp
	var c go_vthunder.FwAlgFtpInstance
	c.DefaultPortDisable = d.Get("default_port_disable").(string)

	SamplingEnableCount := d.Get("sampling_enable.#").(int)
	c.Counters1 = make([]go_vthunder.FwAlgFtpSamplingEnable, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj1 go_vthunder.FwAlgFtpSamplingEnable
