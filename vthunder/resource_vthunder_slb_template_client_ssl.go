...
//vThunder resource SlbTemplateClientSSL
...
func resourceSlbTemplateClientSSLCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received V from method data to SlbTemplateClientSSL --")
		d.SetId(name)
		go_vthunder.PostSlbTemplateClientSSL(client.Token, data, client.Host)
...
func resourceSlbTemplateClientSSLRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetSlbTemplateClientSSL(client.Token, name, client.Host)
		if data == nil {
...
func resourceSlbTemplateClientSSLUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received V from method data to SlbTemplateClientSSL ")
		d.SetId(name)
		go_vthunder.PutSlbTemplateClientSSL(client.Token, name, data, client.Host)
...
func resourceSlbTemplateClientSSLDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_vthunder.DeleteSlbTemplateClientSSL(client.Token, name, client.Host)
		if err != nil {
...
func dataToSlbTemplateClientSSL(d *schema.ResourceData) go_vthunder.ClientSSL {
	var vc go_vthunder.ClientSSL
	var c go_vthunder.ClientSSLInstance

	BypassCertSubjectMultiClassListCount := d.Get("bypass_cert_subject_multi_class_list.#").(int)
	c.BypassCertSubjectMultiClassListName = make([]go_vthunder.BypassCertSubjectMultiClassList, 0, BypassCertSubjectMultiClassListCount)

	for i := 0; i < BypassCertSubjectMultiClassListCount; i++ {
		var obj1 go_vthunder.BypassCertSubjectMultiClassList
	...
	c.CertificateSanContains = make([]go_vthunder.CertificateSanContainsList, 0, CertificateSanContainsListCount)

	for i := 0; i < CertificateSanContainsListCount; i++ {
		var obj2 go_vthunder.CertificateSanContainsList
	...
	c.Equals = make([]go_vthunder.EqualsList, 0, EqualsListCount)

	for i := 0; i < EqualsListCount; i++ {
		var obj3 go_vthunder.EqualsList
	...
	c.ForwardProxyTrustedCa = make([]go_vthunder.ForwardProxyTrustedCaLists, 0, ForwardProxyTrustedCaListsCount)

	for i := 0; i < ForwardProxyTrustedCaListsCount; i++ {
		var obj4 go_vthunder.ForwardProxyTrustedCaLists
	...
	c.Contains = make([]go_vthunder.ContainsList, 0, ContainsListCount)

	for i := 0; i < ContainsListCount; i++ {
		var obj5 go_vthunder.ContainsList
	...
	c.EndsWith = make([]go_vthunder.EndsWithList, 0, EndsWithListCount)

	for i := 0; i < EndsWithListCount; i++ {
		var obj6 go_vthunder.EndsWithList
	...
	c.ClientOcspSrvr = make([]go_vthunder.CaCerts2, 0, CaCertsCount)

	for i := 0; i < CaCertsCount; i++ {
		var obj7 go_vthunder.CaCerts2
	...
	c.ClientAuthContains = make([]go_vthunder.ClientAuthContainsList, 0, ClientAuthContainsListCount)

	for i := 0; i < ClientAuthContainsListCount; i++ {
		var obj8 go_vthunder.ClientAuthContainsList
	...
	c.CertificateSubjectContains = make([]go_vthunder.CertificateSubjectContainsList, 0, CertificateSubjectContainsListCount)

	for i := 0; i < CertificateSubjectContainsListCount; i++ {
		var obj9 go_vthunder.CertificateSubjectContainsList
	...
	c.ClientCertificateRequestCA = make([]go_vthunder.ReqCaLists, 0, ReqCaListsCount)

	for i := 0; i < ReqCaListsCount; i++ {
		var obj10 go_vthunder.ReqCaLists
	...
	c.Counters1 = make([]go_vthunder.SamplingEnable5, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj11 go_vthunder.SamplingEnable5
	...
	c.BypassCertIssuerMultiClassListName = make([]go_vthunder.BypassCertIssuerMultiClassList, 0, BypassCertIssuerMultiClassListCount)

	for i := 0; i < BypassCertIssuerMultiClassListCount; i++ {
		var obj12 go_vthunder.BypassCertIssuerMultiClassList
	...
	c.ClientAuthEquals = make([]go_vthunder.ClientAuthEqualsList, 0, ClientAuthEqualsListCount)

	for i := 0; i < ClientAuthEqualsListCount; i++ {
		var obj13 go_vthunder.ClientAuthEqualsList
	...
	c.CertificateIssuerEquals = make([]go_vthunder.CertificateIssuerEqualsList, 0, CertificateIssuerEqualsListCount)

	for i := 0; i < CertificateIssuerEqualsListCount; i++ {
		var obj14 go_vthunder.CertificateIssuerEqualsList
	...
	c.CertificateSubjectStarts = make([]go_vthunder.CertificateSubjectStartsWithList, 0, CertificateSubjectStartsWithListCount)

	for i := 0; i < CertificateSubjectStartsWithListCount; i++ {
		var obj15 go_vthunder.CertificateSubjectStartsWithList
	...
	c.CertificateSanEndsWith = make([]go_vthunder.CertificateSanEndsWithList, 0, CertificateSanEndsWithListCount)

	for i := 0; i < CertificateSanEndsWithListCount; i++ {
		var obj16 go_vthunder.CertificateSanEndsWithList
	...
	c.CrlShared = make([]go_vthunder.CrlCerts2, 0, CrlCertsCount)

	for i := 0; i < CrlCertsCount; i++ {
		var obj17 go_vthunder.CrlCerts2
	...
	c.MultiClistName = make([]go_vthunder.MultiClassList, 0, MultiClassListCount)

	for i := 0; i < MultiClassListCount; i++ {
		var obj18 go_vthunder.MultiClassList
	...
	c.CertificateIssuerEndsWith = make([]go_vthunder.CertificateIssuerEndsWithList, 0, CertificateIssuerEndsWithListCount)

	for i := 0; i < CertificateIssuerEndsWithListCount; i++ {
		var obj19 go_vthunder.CertificateIssuerEndsWithList
	...
	var obj20 go_vthunder.WebCategory
	...
	c.CertificateSanEquals = make([]go_vthunder.CertificateSanEqualsList, 0, CertificateSanEqualsListCount)

	for i := 0; i < CertificateSanEqualsListCount; i++ {
		var obj21 go_vthunder.CertificateSanEqualsList
	...
	c.CertificateIssuerContains = make([]go_vthunder.CertificateIssuerContainsList, 0, CertificateIssuerContainsListCount)

	for i := 0; i < CertificateIssuerContainsListCount; i++ {
		var obj22 go_vthunder.CertificateIssuerContainsList
	...
	c.BypassCertSanMultiClassListName = make([]go_vthunder.BypassCertSanMultiClassList, 0, BypassCertSanMultiClassListCount)

	for i := 0; i < BypassCertSanMultiClassListCount; i++ {
		var obj23 go_vthunder.BypassCertSanMultiClassList
	...
	c.ClientAuthStartsWith = make([]go_vthunder.ClientAuthStartsWithList, 0, ClientAuthStartsWithListCount)

	for i := 0; i < ClientAuthStartsWithListCount; i++ {
		var obj24 go_vthunder.ClientAuthStartsWithList
	...
	c.CertificateSubjectEndsWith = make([]go_vthunder.CertificateSubjectEndsWithList, 0, CertificateSubjectEndsWithListCount)

	for i := 0; i < CertificateSubjectEndsWithListCount; i++ {
		var obj25 go_vthunder.CertificateSubjectEndsWithList
	...
	c.Ec = make([]go_vthunder.EcList2, 0, EcListCount)

	for i := 0; i < EcListCount; i++ {
		var obj26 go_vthunder.EcList2
	...
	c.ServerShared = make([]go_vthunder.ServerNameList, 0, ServerNameListCount)

	for i := 0; i < ServerNameListCount; i++ {
		var obj27 go_vthunder.ServerNameList
	...
	c.CertificateIssuerStarts = make([]go_vthunder.CertificateIssuerStartsWithList, 0, CertificateIssuerStartsWithListCount)

	for i := 0; i < CertificateIssuerStartsWithListCount; i++ {
		var obj28 go_vthunder.CertificateIssuerStartsWithList
	...
	c.CertificateSanStarts = make([]go_vthunder.CertificateSanStartsWithList, 0, CertificateSanStartsWithListCount)

	for i := 0; i < CertificateSanStartsWithListCount; i++ {
		var obj29 go_vthunder.CertificateSanStartsWithList
	...
	c.ClientAuthEndsWith = make([]go_vthunder.ClientAuthEndsWithList, 0, ClientAuthEndsWithListCount)

	for i := 0; i < ClientAuthEndsWithListCount; i++ {
		var obj30 go_vthunder.ClientAuthEndsWithList
	...
	c.CertificateSubjectEquals = make([]go_vthunder.CertificateSubjectEqualsList, 0, CertificateSubjectEqualsListCount)

	for i := 0; i < CertificateSubjectEqualsListCount; i++ {
		var obj31 go_vthunder.CertificateSubjectEqualsList
	...
	c.CipherWoPrio = make([]go_vthunder.CipherWithoutPrioList2, 0, CipherWithoutPrioListCount)

	for i := 0; i < CipherWithoutPrioListCount; i++ {
		var obj32 go_vthunder.CipherWithoutPrioList2
	...
	c.StartsWith = make([]go_vthunder.StartsWithList, 0, StartsWithListCount)

	for i := 0; i < StartsWithListCount; i++ {
		var obj33 go_vthunder.StartsWithList
