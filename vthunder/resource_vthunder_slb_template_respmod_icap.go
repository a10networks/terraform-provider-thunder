...
//vThunder resource SlbTemplateRespmodIcap
...
func resourceSlbTemplateRespmodIcapCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received V from method data to SlbTemplateRespmodIcap --")
		d.SetId(name)
		go_vthunder.PostSlbTemplateRespmodIcap(client.Token, data, client.Host)
...
func resourceSlbTemplateRespmodIcapRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetSlbTemplateRespmodIcap(client.Token, name, client.Host)
		if data == nil {
...
func resourceSlbTemplateRespmodIcapUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received V from method data to SlbTemplateRespmodIcap ")
		d.SetId(name)
		go_vthunder.PutSlbTemplateRespmodIcap(client.Token, name, data, client.Host)
...
func resourceSlbTemplateRespmodIcapDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_vthunder.DeleteSlbTemplateRespmodIcap(client.Token, name, client.Host)
		if err != nil {
...
func dataToSlbTemplateRespmodIcap(d *schema.ResourceData) go_vthunder.RespmodIcap {
	var vc go_vthunder.RespmodIcap
	var c go_vthunder.RespmodIcapInstance
	...
	c.Mask = make([]go_vthunder.BypassIPCfg2, 0, BypassIpCfgCount)

	for i := 0; i < BypassIpCfgCount; i++ {
		var obj1 go_vthunder.BypassIPCfg2
