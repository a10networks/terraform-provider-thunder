...
//vThunder resource FwTcpWindowCheck
...
func resourceFwTcpWindowCheckCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		go_vthunder.PostFwTcpWindowCheck(client.Token, data, client.Host)
...
func resourceFwTcpWindowCheckRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetFwTcpWindowCheck(client.Token, client.Host)
		if data == nil {
...
func dataToFwTcpWindowCheck(d *schema.ResourceData) go_vthunder.FwTcpWindowCheck {
	var vc go_vthunder.FwTcpWindowCheck
	var c go_vthunder.FwTCPWindowCheckInstance
	c.Status = d.Get("status").(string)

	SamplingEnableCount := d.Get("sampling_enable.#").(int)
	c.Counters1 = make([]go_vthunder.FwTCPWindowCheckSamplingEnable, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj1 go_vthunder.FwTCPWindowCheckSamplingEnable
