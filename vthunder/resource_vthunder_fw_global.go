...
func resourceFwGlobalCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		go_vthunder.PostFwGlobal(client.Token, data, client.Host)
...
func resourceFwGlobalRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetFwGlobal(client.Token, client.Host)
		if data == nil {
...
func dataToFwGlobal(d *schema.ResourceData) go_vthunder.FwGlobal {
	var vc go_vthunder.FwGlobal
	var c go_vthunder.FwGlobalInstance
	...
	c.DisableApplicationProtocol = make([]go_vthunder.FwGlobalDisableAppList, 0, DisableAppListCount)

	for i := 0; i < DisableAppListCount; i++ {
		var obj1 go_vthunder.FwGlobalDisableAppList
	...
	c.Counters1 = make([]go_vthunder.FwGlobalSamplingEnable, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj2 go_vthunder.FwGlobalSamplingEnable
