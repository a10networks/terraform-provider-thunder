...
//vThunder resource FwTemplateLogging
...
func resourceFwTemplateLoggingCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		go_vthunder.PostFwTemplateLogging(client.Token, data, client.Host)
...
func resourceFwTemplateLoggingRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetFwTemplateLogging(client.Token, name, client.Host)
		if data == nil {
...
func dataToFwTemplateLogging(d *schema.ResourceData) go_vthunder.FwTemplateLogging {
	var vc go_vthunder.FwTemplateLogging
	var c go_vthunder.FwTemplateLoggingInstance

	var obj1 go_vthunder.FwTemplateLoggingIncludeHTTP
	prefix := "include_http.0."

	HeaderCfgCount := d.Get(prefix + "header_cfg.#").(int)
	obj1.CustomMaxLength = make([]go_vthunder.FwTemplateLoggingHeaderCfg, 0, HeaderCfgCount)

	for i := 0; i < HeaderCfgCount; i++ {
		var obj1_1 go_vthunder.FwTemplateLoggingHeaderCfg
		prefix1 := fmt.Sprintf(prefix+"header_cfg.%d.", i)
	...
	var obj1_2 go_vthunder.FwTemplateLoggingSourceAddress
	...
	var obj1_3 go_vthunder.FwTemplateLoggingLog
	prefix3 := "log.0."
	...
	var obj1_4 go_vthunder.FwTemplateLoggingIncludeRadiusAttribute
	...
	obj1_4.NoQuote = d.Get(prefix + "no_quote").(int)

	AttrCfgCount := d.Get(prefix4 + "attr_cfg.#").(int)
	obj1_4.AttrEvent = make([]go_vthunder.FwTemplateLoggingAttrCfg, 0, AttrCfgCount)

	for i := 0; i < AttrCfgCount; i++ {
		var obj1_4_1 go_vthunder.FwTemplateLoggingAttrCfg
		prefix4_1 := fmt.Sprintf(prefix4+"attr_cfg.%d.", i)
	...
	var obj1_5 go_vthunder.FwTemplateLoggingRule
	prefix5 := "rule.0."

	var obj1_5_1 go_vthunder.FwTemplateLoggingRuleHTTPRequests
	...
	obj1_5_1.IncludeByteCount = make([]go_vthunder.FwTemplateLoggingDestPort, 0, DestPortCount)

	for i := 0; i < DestPortCount; i++ {
		var obj1_5_1_1 go_vthunder.FwTemplateLoggingDestPort
