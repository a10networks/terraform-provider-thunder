...
//vThunder resource SlbTemplateDiameter
...
func resourceSlbTemplateDiameterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received V from method data to SlbTemplateDiameter --")
		d.SetId(name)
		go_vthunder.PostSlbTemplateDiameter(client.Token, data, client.Host)
...
func resourceSlbTemplateDiameterRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetSlbTemplateDiameter(client.Token, name, client.Host)
		if data == nil {
...
func resourceSlbTemplateDiameterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received V from method data to SlbTemplateDiameter ")
		d.SetId(name)
		go_vthunder.PutSlbTemplateDiameter(client.Token, name, data, client.Host)
...
func resourceSlbTemplateDiameterDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_vthunder.DeleteSlbTemplateDiameter(client.Token, name, client.Host)
		if err != nil {
...
func dataToSlbTemplateDiameter(d *schema.ResourceData) go_vthunder.Diameter {
	var vc go_vthunder.Diameter
	var c go_vthunder.DiameterInstance
	...
	c.MessageCode = make([]go_vthunder.MessageCodeList, 0, MessageCodeListCount)
	for i := 0; i < MessageCodeListCount; i++ {
		var mcl go_vthunder.MessageCodeList
	...
	c.Avp = make([]go_vthunder.AvpList, 0, AvpListCount)
	for i := 0; i < AvpListCount; i++ {
		var al go_vthunder.AvpList
