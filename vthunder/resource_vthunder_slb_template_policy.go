...
//vThunder resource SlbTemplatePolicy
...
func resourceSlbTemplatePolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_vthunder.GetSlbTemplatePolicy(client.Token, name, client.Host)
		if data == nil {
...
func resourceSlbTemplatePolicyDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_vthunder.DeleteSlbTemplatePolicy(client.Token, name, client.Host)
		if err != nil {
...
func dataToSlbTemplatePolicy(d *schema.ResourceData) go_vthunder.Policy {
	var vc go_vthunder.Policy
	var c go_vthunder.PolicyInstance

	var obj1 go_vthunder.ForwardPolicy
	prefix1 := "forward_policy.0."

	FilteringCount := d.Get(prefix1 + "filtering.#").(int)
	obj1.SsliURLFiltering = make([]go_vthunder.Filtering, 0, FilteringCount)

	for i := 0; i < FilteringCount; i++ {
		var obj1_1 go_vthunder.Filtering
	...
	obj1.SsliURLFilteringSan = make([]go_vthunder.SanFiltering, 0, SanFilteringCount)

	for i := 0; i < SanFilteringCount; i++ {
		var obj1_2 go_vthunder.SanFiltering
	...
	obj1.Log = make([]go_vthunder.ActionList, 0, ActionListCount)
	...
	for i := 0; i < ActionListCount; i++ {
		...
		obj1_3.Counters1 = make([]go_vthunder.SamplingEnable3, 0, SamplingEnableCount)

		for j := 0; j < SamplingEnableCount; j++ {
			var obj1_3_1 go_vthunder.SamplingEnable3
	...
	obj1.MatchAny = make([]go_vthunder.SourceList, 0, SourceListCount)

	for i := 0; i < SourceListCount; i++ {
		var obj1_4 go_vthunder.SourceList
		...
		var obj1_4_1 go_vthunder.Destination
		prefix3 := prefix2 + "destination.0."

		ClassListListCount := d.Get(prefix3 + "class_list_list.#").(int)
		obj1_4_1.UUID = make([]go_vthunder.ClassListList, 0, ClassListListCount)

		for j := 0; j < ClassListListCount; j++ {
			var obj1_4_1_1 go_vthunder.ClassListList
			...
			obj1_4_1_1.Counters1 = make([]go_vthunder.SamplingEnable3, 0, SamplingEnableCount)

			for k := 0; k < SamplingEnableCount; k++ {
				var obj1_4_1_1_1 go_vthunder.SamplingEnable3
		...
		obj1_4_1.WebCategoryList = make([]go_vthunder.WebCategoryListList, 0, WebCategoryListListCount)

		for j := 0; j < WebCategoryListListCount; j++ {
			var obj1_4_1_2 go_vthunder.WebCategoryListList
			...
			SamplingEnableCount := d.Get(prefix4 + "sampling_enable.#").(int)
			obj1_4_1_2.Counters1 = make([]go_vthunder.SamplingEnable3, 0, SamplingEnableCount)

			for k := 0; k < SamplingEnableCount; k++ {
				var obj1_4_1_2_1 go_vthunder.SamplingEnable3
				prefix5 := fmt.Sprintf(prefix4+"sampling_enable.%d.", k)
		...
		var obj1_4_1_3 go_vthunder.Any
		prefix4 := prefix3 + "any.0."
		obj1_4_1_3.Action = d.Get(prefix4 + "action").(string)

		SamplingEnableCount := d.Get(prefix4 + "sampling_enable.#").(int)
		obj1_4_1_3.Counters1 = make([]go_vthunder.SamplingEnable3, 0, SamplingEnableCount)

		for j := 0; j < SamplingEnableCount; j++ {
			var obj1_4_1_3_1 go_vthunder.SamplingEnable3
		...
		obj1_4.Counters1 = make([]go_vthunder.SamplingEnable3, 0, SamplingEnableCount1)

		for j := 0; j < SamplingEnableCount1; j++ {
			var obj1_4_2 go_vthunder.SamplingEnable3
			prefix3 := fmt.Sprintf(prefix2+"sampling_enable.%d.", j)
	...
	for i := 0; i < LidListCount; i++ {
		...
		var obj2_1_1 go_vthunder.DNS64
		...
		obj2_1.Threshold = make([]go_vthunder.ResponseCodeRateLimit, 0, ResponseCodeRateLimitCount)

		for j := 0; j < ResponseCodeRateLimitCount; j++ {
			var obj2_1_2 go_vthunder.ResponseCodeRateLimit
	...
	c.Counters1 = make([]go_vthunder.SamplingEnable3, 0, SamplingEnableCount)

	for i := 0; i < SamplingEnableCount; i++ {
		var obj3 go_vthunder.SamplingEnable3
	...
	c.PbslbInterval = make([]go_vthunder.BwListID, 0, BwListIDCount)

	for i := 0; i < BwListIDCount; i++ {
		var obj4 go_vthunder.BwListID
