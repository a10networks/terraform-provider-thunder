...
func resourcePartitionCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		d.SetId(name)
		go_vthunder.PostPartition(client.Token, v, client.Host)
...
func resourcePartitionRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		partition, err := go_vthunder.GetPartition(client.Token, name, client.Host)
...
func resourcePartitionUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		v := dataToPartition(name, d)
		d.SetId(name)
		go_vthunder.PutPartition(client.Token, name, v, client.Host)
...
func dataToPartition(name string, d *schema.ResourceData) go_vthunder.Partition {
