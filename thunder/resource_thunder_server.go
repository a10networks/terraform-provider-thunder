...
func dataToServer(name string, d *schema.ResourceData) go_thunder.Server {
	//	logger := util.GetLoggerInstance()
	...
	sInstance.PortNumber = make([]go_thunder.PortLists, 0, portListCount)
	for i := 0; i < portListCount; i++ {
		var pl go_thunder.PortLists
		...
		pl.Counters1 = make([]go_thunder.SamplingEnable, sampleCount, sampleCount)
		...
		pl.AlternateName = make([]go_thunder.AlternatePort, altPortCt, altPortCt)

		for x := 0; x < altPortCt; x++ {
			var alt go_thunder.AlternatePort
	...
	sInstance.AlternateName = make([]go_thunder.AlternateServer, 0, altServerCt)

	for i := 0; i < altServerCt; i++ {
		var altSer go_thunder.AlternateServer
