...
//Thunder resource overlay tunnel vtep
...
func resourceVtepCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		v := dataToVtep(d)
		d.SetId(name)
		go_thunder.PostVtep(client.Token, v, client.Host)
...
func resourceVtepRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		vtep, err := go_thunder.GetVtep(client.Token, name, client.Host)
...
func resourceVtepUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		v := dataToVtep(d)
		d.SetId(name)
		go_thunder.PutVtep(client.Token, name, v, client.Host)
...
func resourceVtepDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_thunder.DeleteVtep(client.Token, name, client.Host)
		if err != nil {
...
func dataToVtep(d *schema.ResourceData) go_thunder.Vtep {
	var s go_thunder.Vtep

	var sInstance go_thunder.VtepInstance
	...
	sInstance.Counters1 = make([]go_thunder.SamplingEnableOT, 0, samplingCount)
	for i := 0; i < samplingCount; i++ {
		var samp go_thunder.SamplingEnableOT
	...
	sInstance.IPAddr = make([]go_thunder.HostList, 0, hostListCount)

	for i := 0; i < hostListCount; i++ {
		var host go_thunder.HostList
	...
	sInstance.Segment = make([]go_thunder.DestinationIPAddressList, 0, destAddrCnt)
	for i := 0; i < destAddrCnt; i++ {
		var destAddrr go_thunder.DestinationIPAddressList
