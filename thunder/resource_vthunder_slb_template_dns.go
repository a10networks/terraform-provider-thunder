...
func dataToTemplateDNS(d *schema.ResourceData) go_thunder.DNS {
	var vc go_thunder.DNS
	var c go_thunder.DNSInstance

	c.Name = d.Get("name").(string)
	c.DnssecServiceGroup = d.Get("dnssec_service_group").(string)
	var classListObj go_thunder.ClassList

	lidListCount := d.Get("class_list.0.lid_list.#").(int)
	classListObj.ActionValue = make([]go_thunder.LidList, 0, lidListCount)

	for i := 0; i < lidListCount; i++ {
		var cl go_thunder.LidList
	...
	var RateLimiting go_thunder.ResponseRateLimiting
