...
func resourceSlbTemplateHTTPCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		d.SetId(name)
		go_thunder.PostSlbTemplateHTTP(client.Token, data, client.Host)
...
func resourceSlbTemplateHTTPRead(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		data, err := go_thunder.GetSlbTemplateHTTP(client.Token, name, client.Host)
		if data == nil {
...
func resourceSlbTemplateHTTPUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		logger.Println("[INFO] received formatted data from method data to SlbTemplateHTTP ")
		d.SetId(name)
		go_thunder.PutSlbTemplateHTTP(client.Token, name, data, client.Host)
...
func resourceSlbTemplateHTTPDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if client.Host != "" {
		...
		err := go_thunder.DeleteSlbTemplateHTTP(client.Token, name, client.Host)
		if err != nil {
...
func dataToSlbTemplateHTTP(d *schema.ResourceData) go_thunder.HTTP {
	var vc go_thunder.HTTP
	var c go_thunder.HTTPInstance
	...
	c.ExcludeURI = make([]go_thunder.CompressionExcludeURI, 0, CompressionExcludeURICount)

	for i := 0; i < CompressionExcludeURICount; i++ {
		var obj1 go_thunder.CompressionExcludeURI
	...
	var obj2 go_thunder.RedirectRewrite
	...
	obj2.RewriteTo = make([]go_thunder.MatchList, 0, MatchListCount)

	for i := 0; i < MatchListCount; i++ {
		var obj2_1 go_thunder.MatchList
	...
	c.RequestHeaderErase = make([]go_thunder.RequestHeaderEraseList, 0, RequestHeaderEraseListCount)

	for i := 0; i < RequestHeaderEraseListCount; i++ {
		var obj3 go_thunder.RequestHeaderEraseList
	...
	c.HostSwitchingType = make([]go_thunder.HostSwitching, 0, HostSwitchingCount)

	for i := 0; i < HostSwitchingCount; i++ {
		var obj4 go_thunder.HostSwitching
	...
	c.ResponseHeaderInsertType = make([]go_thunder.ResponseHeaderInsertList, 0, ResponseHeaderInsertListCount)

	for i := 0; i < ResponseHeaderInsertListCount; i++ {
		var obj5 go_thunder.ResponseHeaderInsertList
	...
	c.URLServiceGroup = make([]go_thunder.URLSwitching, 0, URLSwitchingCount)

	for i := 0; i < URLSwitchingCount; i++ {
		var obj7 go_thunder.URLSwitching
	...
	c.ResponseNewString = make([]go_thunder.ResponseContentReplaceList, 0, ResponseContentReplaceListCount)

	for i := 0; i < ResponseContentReplaceListCount; i++ {
		var obj8 go_thunder.ResponseContentReplaceList
	...
	c.RequestHeaderInsert = make([]go_thunder.RequestHeaderInsertList, 0, RequestHeaderInsertListCount)

	for i := 0; i < RequestHeaderInsertListCount; i++ {
		var obj9 go_thunder.RequestHeaderInsertList
	...
	c.ResponseHeaderErase = make([]go_thunder.ResponseHeaderEraseList, 0, ResponseHeaderEraseListCount)

	for i := 0; i < ResponseHeaderEraseListCount; i++ {
		var obj10 go_thunder.ResponseHeaderEraseList
	...
	c.ContentType = make([]go_thunder.CompressionContentType, 0, CompressionContentTypeCount)

	for i := 0; i < CompressionContentTypeCount; i++ {
		var obj11 go_thunder.CompressionContentType
	...
	c.ExcludeContentType = make([]go_thunder.CompressionExcludeContentType, 0, CompressionExcludeContentTypeCount)

	for i := 0; i < CompressionExcludeContentTypeCount; i++ {
		var obj12 go_thunder.CompressionExcludeContentType
