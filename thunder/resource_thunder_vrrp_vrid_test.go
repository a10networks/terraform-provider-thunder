		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("thunder_vrrp_vrid.vrrp_vrid", "vrid_val", "1"),
					resource.TestCheckResourceAttr("thunder_vrrp_vrid.vrrp_vrid", "floating_ip.0.ip_address_cfg.0.ip_address", "1.1.1.10"),
					resource.TestCheckResourceAttr("thunder_vrrp_vrid.vrrp_vrid", "floating_ip.0.ip_address_cfg.1.ip_address", "1.1.1.11"),
					resource.TestCheckResourceAttr("thunder_vrrp_vrid.vrrp_vrid", "blade_parameters.0.priority", "250"),
				),
			},
		},
	})
...
func TestAccThunderVrrpVrid_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAcctPreCheck(t)
		},
...
func TestDataToVrrpVrid(t *testing.T) {
	...
	var sInstance go_thunder.Vrid
	var s go_thunder.VridInstance

	var floating_ip go_thunder.FloatingIP

	floating_ip.IPAddress = make([]go_thunder.IPAddressCfg, 0, 1)

	var blade_parameters go_thunder.BladeParameters
	...
	cases := []struct {
		...
		output go_thunder.VridInstance
	}{
		{
...
func testCheckVrrpVridDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		name := rs.Primary.ID
		sr, err := go_thunder.GetVrrpVrid(client.Token, name, client.Host)
		if err != nil {
			return err
		}
		if sr == nil {
