		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("thunder_service_group.sg9", "name", NAME_SG),
					resource.TestCheckResourceAttr("thunder_service_group.sg9", "protocol", "TCP"),
					resource.TestCheckResourceAttr("thunder_service_group.sg9", "member_list.0.name", SR_NAME),
					resource.TestCheckResourceAttr("thunder_service_group.sg9", "member_list.0.port", "80"),
				),
			},
		},
	})
...
func TestAccThunderServiceGroup_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAcctPreCheck(t)
		},
...
func TestDataToSg(t *testing.T) {
	...
	sInstance.Priority = make([]go_thunder.Priorities, 0, 1)
	sInstance.Counters1 = make([]go_thunder.SamplingEnable, 0, 1)
	sInstance.Host = make([]go_thunder.MemberList, 0, 1)

	s.Name = sInstance

	cases := []struct {
		input  *schema.ResourceData
		output go_thunder.ServiceGroup
	}{
		{
