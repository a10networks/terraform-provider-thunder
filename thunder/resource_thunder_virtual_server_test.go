		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("thunder_virtual_server.vs9", "name", NAME_VS),
					resource.TestCheckResourceAttr("thunder_virtual_server.vs9", "ha_dynamic", "1"),
					resource.TestCheckResourceAttr("thunder_virtual_server.vs9", "ip_address", "10.0.9.7"),
					resource.TestCheckResourceAttr("thunder_virtual_server.vs9", "port_list.0.auto", "1"),
					resource.TestCheckResourceAttr("thunder_virtual_server.vs9", "port_list.0.service_group", SG),
					resource.TestCheckResourceAttr("thunder_virtual_server.vs9", "port_list.0.snat_on_vip", "1"),
					resource.TestCheckResourceAttr("thunder_virtual_server.vs9", "port_list.0.port_number", "8080"),
					resource.TestCheckResourceAttr("thunder_virtual_server.vs9", "port_list.0.protocol", "tcp"),
				),
			},
		},
	})
...
func TestAccThunderVirtualServer_import(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck: func() {
			testAcctPreCheck(t)
		},
...
func TestDataToVs(t *testing.T) {
	...
	vsMain.Protocol = make([]go_thunder.PortList, 0, 1)

	v.Name = vsMain

	cases := []struct {
		input  *schema.ResourceData
		output go_thunder.VirtalServerInstanceMain
	}{
		{
...
func testCheckVirtualServerDestroyed(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		name := rs.Primary.ID
		device, err := go_thunder.GetVS(client.Token, name, client.Host)
		if err != nil {
			return err
		}
		if device == nil {
