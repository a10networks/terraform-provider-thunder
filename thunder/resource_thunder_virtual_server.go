...
func dataToVs(name string, d *schema.ResourceData) go_thunder.VirtalServerInstanceMain {
	...
	for i := 0; i < portListCount; i++ {
		...
		pl.TemplateClientSsl = d.Get(prefix + ".template_client_ssl").(string)
		pl.TemplateServerSsl = d.Get(prefix + ".template_server_ssl").(string)
		pl.TemplatePersistSourceIP = d.Get(prefix + ".template_persist_source_ip").(string)
		pl.StatsDataAction = d.Get(prefix + ".stats_data_action").(string)
		pl.PersistType = d.Get(prefix + ".persist_type").(string)
		pl.TemplateTCPProxy = d.Get(prefix + ".template_tcp_proxy").(string)
