...
type DdosDstEntrySrcPortOper struct {
    
    Oper DdosDstEntrySrcPortOperOper `json:"oper"`
    PortNum int `json:"port-num"`
    Protocol string `json:"protocol"`
    DstEntryName string 

}
type DataDdosDstEntrySrcPortOper struct {
    DtDdosDstEntrySrcPortOper DdosDstEntrySrcPortOper `json:"src-port"`
}


type DdosDstEntrySrcPortOperOper struct {
    Ddos_entry_list []DdosDstEntrySrcPortOperOperDdos_entry_list `json:"ddos_entry_list"`
...
type DdosDstEntrySrcPortOperOperDdos_entry_list struct {
...
func (p *DdosDstEntrySrcPortOper) GetId() string{
    return "1"
}

func (p *DdosDstEntrySrcPortOper) getPath() string{
    
    return "ddos/dst/entry/" +p.DstEntryName + "/src-port/" +strconv.Itoa(p.PortNum)+"+"+p.Protocol+"/oper"
}

func (p *DdosDstEntrySrcPortOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDstEntrySrcPortOper,error) {
logger.Println("DdosDstEntrySrcPortOper::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosDstEntrySrcPortOper
    if err == nil {
        if len(axResp) > 0{
