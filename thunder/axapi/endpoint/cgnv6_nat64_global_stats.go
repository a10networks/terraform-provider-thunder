...
type DataCgnv6Nat64GlobalStats struct {
    DtCgnv6Nat64GlobalStats Cgnv6Nat64GlobalStats `json:"global"`
}


type Cgnv6Nat64GlobalStatsStats struct {
    ...
    L3 int `json:"l3"`
    Source_matches_prefix int `json:"source_matches_prefix"`
...
func (p *Cgnv6Nat64GlobalStats) getPath() string{
    return "cgnv6/nat64/global/stats"
}

func (p *Cgnv6Nat64GlobalStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataCgnv6Nat64GlobalStats,error) {
logger.Println("Cgnv6Nat64GlobalStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataCgnv6Nat64GlobalStats
    if err == nil {
        if len(axResp) > 0{
