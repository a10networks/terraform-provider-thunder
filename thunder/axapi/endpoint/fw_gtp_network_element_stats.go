...
type FwGtpNetworkElementStats struct {
	Stats FwGtpNetworkElementStatsStats `json:"stats"`
}
type DataFwGtpNetworkElementStats struct {
	DtFwGtpNetworkElementStats FwGtpNetworkElementStats `json:"network-element"`
}

type FwGtpNetworkElementStatsStats struct {
...
func (p *FwGtpNetworkElementStats) GetId() string {
	return "1"
}

func (p *FwGtpNetworkElementStats) getPath() string {
	return "fw/gtp/network-element/stats"
}

func (p *FwGtpNetworkElementStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataFwGtpNetworkElementStats, error) {
	logger.Println("FwGtpNetworkElementStats::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataFwGtpNetworkElementStats
	if err == nil {
		if len(axResp) > 0 {
