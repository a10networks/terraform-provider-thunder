...
type SlbSslCountersOper struct {
    
    Oper SlbSslCountersOperOper `json:"oper"`

}
type DataSlbSslCountersOper struct {
    DtSlbSslCountersOper SlbSslCountersOper `json:"ssl-counters"`
}


type SlbSslCountersOperOper struct {
    ...
    SniAutomapSuccesses int `json:"sni-automap-successes"`
    SniAutomapFailures int `json:"sni-automap-failures"`
    SniAutomapConnClosed int `json:"sni-automap-conn-closed"`
    SniAutomapMaxActiveConn int `json:"sni-automap-max-active-conn"`
    SniAutomapMissingCert int `json:"sni-automap-missing-cert"`
    SniBypassMissingCert int `json:"sni-bypass-missing-cert"`
    SniBypassExpiredCert int `json:"sni-bypass-expired-cert"`
    SniBypassExplicitList int `json:"sni-bypass-explicit-list"`
...
func (p *SlbSslCountersOper) GetId() string{
    return "1"
}

func (p *SlbSslCountersOper) getPath() string{
    return "slb/ssl-counters/oper"
}

func (p *SlbSslCountersOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbSslCountersOper,error) {
logger.Println("SlbSslCountersOper::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbSslCountersOper
    if err == nil {
        if len(axResp) > 0{
