...
type DdosDstZonePortZoneServiceOtherPortIndStats struct {
    
    Stats DdosDstZonePortZoneServiceOtherPortIndStatsStats `json:"stats"`
    PortOther string 
    ZoneName string 
    Protocol string 

}
type DataDdosDstZonePortZoneServiceOtherPortIndStats struct {
    DtDdosDstZonePortZoneServiceOtherPortIndStats DdosDstZonePortZoneServiceOtherPortIndStats `json:"port-ind"`
}


type DdosDstZonePortZoneServiceOtherPortIndStatsStats struct {
...
func (p *DdosDstZonePortZoneServiceOtherPortIndStats) GetId() string{
    return "1"
}

func (p *DdosDstZonePortZoneServiceOtherPortIndStats) getPath() string{
    
    return "ddos/dst/zone/" +p.ZoneName + "/port/zone-service-other/" +p.PortOther + "+" +p.Protocol + "/port-ind/stats"
}

func (p *DdosDstZonePortZoneServiceOtherPortIndStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDstZonePortZoneServiceOtherPortIndStats,error) {
logger.Println("DdosDstZonePortZoneServiceOtherPortIndStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosDstZonePortZoneServiceOtherPortIndStats
    if err == nil {
        if len(axResp) > 0{
