...
type DdosDstEntrySrcPortStats21 struct {
	Inst struct {

    PortNum int `json:"port-num"`
    Protocol string `json:"protocol"`
    Stats DdosDstEntrySrcPortStats21Stats `json:"stats"`
    DstEntryName string 
...
type DdosDstEntrySrcPortStats21Stats struct {
    UdpPort DdosDstEntrySrcPortStats21StatsUdpPort `json:"udp-port"`
}


type DdosDstEntrySrcPortStats21StatsUdpPort struct {
...
func (p *DdosDstEntrySrcPortStats21) GetId() string{
    return "1"
}

func (p *DdosDstEntrySrcPortStats21) getPath() string{
    return "ddos/dst/entry/" +p.Inst.DstEntryName + "/src-port/" +strconv.Itoa(p.Inst.PortNum)+"+"+p.Inst.Protocol+"/stats?udp-port=true"
}

func (p *DdosDstEntrySrcPortStats21) Post(authToken string, host string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntrySrcPortStats21::Post")
...
func (p *DdosDstEntrySrcPortStats21) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntrySrcPortStats21::Get")
    headers := axapi.GenRequestHeader(authToken)
...
func (p *DdosDstEntrySrcPortStats21) Put(authToken string, host string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntrySrcPortStats21::Put")
    headers := axapi.GenRequestHeader(authToken)
    payloadBytes, err := axapi.SerializeToJson(p)
    if err != nil {
        logger.Println("Failed to serialize struct as json", err)
...
func (p *DdosDstEntrySrcPortStats21) Delete(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntrySrcPortStats21::Delete")
    headers := axapi.GenRequestHeader(authToken)
