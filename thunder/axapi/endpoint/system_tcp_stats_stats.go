...
type DataSystemTcpStatsStats struct {
    DtSystemTcpStatsStats SystemTcpStatsStats `json:"tcp-stats"`
}


type SystemTcpStatsStatsStats struct {
    ...
    Atcprexmitsadrop int `json:"atcprexmitsadrop"`
...
func (p *SystemTcpStatsStats) getPath() string{
    return "system/tcp-stats/stats"
}

func (p *SystemTcpStatsStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSystemTcpStatsStats,error) {
logger.Println("SystemTcpStatsStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSystemTcpStatsStats
    if err == nil {
        if len(axResp) > 0{
