...
type SlbVirtualServerPortStats57 struct {
	PortNumber int `json:"port-number"`

	Protocol string `json:"protocol"`

	Stats SlbVirtualServerPortStats57Stats `json:"stats"`

	Virtual_server_name string
}
type DataSlbVirtualServerPortStats57 struct {
	DtSlbVirtualServerPortStats57 SlbVirtualServerPortStats57 `json:"port"`
}

type SlbVirtualServerPortStats57Stats struct {
...
func (p *SlbVirtualServerPortStats57) GetId() string {
	return "1"
}

func (p *SlbVirtualServerPortStats57) getPath() string {

	return "slb/virtual-server/" + p.Virtual_server_name + "/port/" + strconv.Itoa(p.PortNumber) + "+" + p.Protocol + "/stats"
}

func (p *SlbVirtualServerPortStats57) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbVirtualServerPortStats57, error) {
	logger.Println("SlbVirtualServerPortStats57::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataSlbVirtualServerPortStats57
	if err == nil {
		if len(axResp) > 0 {
