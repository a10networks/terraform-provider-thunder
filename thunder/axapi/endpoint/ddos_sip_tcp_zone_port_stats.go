...
type DdosSipTcpZonePortStats struct {
    
    Stats DdosSipTcpZonePortStatsStats `json:"stats"`

}
type DataDdosSipTcpZonePortStats struct {
    DtDdosSipTcpZonePortStats DdosSipTcpZonePortStats `json:"sip-tcp-zone-port"`
}


type DdosSipTcpZonePortStatsStats struct {
...
func (p *DdosSipTcpZonePortStats) GetId() string{
    return "1"
}

func (p *DdosSipTcpZonePortStats) getPath() string{
    return "ddos/sip-tcp-zone-port/stats"
}

func (p *DdosSipTcpZonePortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosSipTcpZonePortStats,error) {
logger.Println("DdosSipTcpZonePortStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosSipTcpZonePortStats
    if err == nil {
        if len(axResp) > 0{
