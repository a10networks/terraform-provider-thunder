...
type IpFragStats struct {
    
    Stats IpFragStatsStats `json:"stats"`

}
type DataIpFragStats struct {
    DtIpFragStats IpFragStats `json:"frag"`
}


type IpFragStatsStats struct {
...
func (p *IpFragStats) GetId() string{
    return "1"
}

func (p *IpFragStats) getPath() string{
    return "ip/frag/stats"
}

func (p *IpFragStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataIpFragStats,error) {
logger.Println("IpFragStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataIpFragStats
    if err == nil {
        if len(axResp) > 0{
