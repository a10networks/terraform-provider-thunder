...
type DataSystemAppPerformanceStats struct {
    DtSystemAppPerformanceStats SystemAppPerformanceStats `json:"app-performance"`
}
...
func (p *SystemAppPerformanceStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSystemAppPerformanceStats,error) {
logger.Println("SystemAppPerformanceStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSystemAppPerformanceStats
    if err == nil {
        if len(axResp) > 0{
