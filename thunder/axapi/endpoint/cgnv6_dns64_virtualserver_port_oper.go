...
type Cgnv6Dns64VirtualserverPortOper struct {
	Oper Cgnv6Dns64VirtualserverPortOperOper `json:"oper"`

	PortNumber int `json:"port-number"`

	Protocol string `json:"protocol"`

	Dns64_virtualserver_name string
}
type DataCgnv6Dns64VirtualserverPortOper struct {
	DtCgnv6Dns64VirtualserverPortOper Cgnv6Dns64VirtualserverPortOper `json:"port"`
}

type Cgnv6Dns64VirtualserverPortOperOper struct {
	...
	HttpHitsList         []Cgnv6Dns64VirtualserverPortOperOperHttpHitsList     `json:"http-hits-list"`
	HttpVportCpuList     []Cgnv6Dns64VirtualserverPortOperOperHttpVportCpuList `json:"http-vport-cpu-list"`
...
type Cgnv6Dns64VirtualserverPortOperOperHttpHitsList struct {
	Name      string `json:"name"`
	HitsCount int    `json:"hits-count"`
}

type Cgnv6Dns64VirtualserverPortOperOperHttpVportCpuList struct {
...
func (p *Cgnv6Dns64VirtualserverPortOper) GetId() string {
	return "1"
}

func (p *Cgnv6Dns64VirtualserverPortOper) getPath() string {

	return "cgnv6/dns64-virtualserver/" + p.Dns64_virtualserver_name + "/port/" + strconv.Itoa(p.PortNumber) + "+" + p.Protocol + "/oper"
}

func (p *Cgnv6Dns64VirtualserverPortOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataCgnv6Dns64VirtualserverPortOper, error) {
	logger.Println("Cgnv6Dns64VirtualserverPortOper::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataCgnv6Dns64VirtualserverPortOper
	if err == nil {
		if len(axResp) > 0 {
