...
type DdosUdpPortStats struct {
    
    Stats DdosUdpPortStatsStats `json:"stats"`

}
type DataDdosUdpPortStats struct {
    DtDdosUdpPortStats DdosUdpPortStats `json:"udp-port"`
}


type DdosUdpPortStatsStats struct {
...
func (p *DdosUdpPortStats) GetId() string{
    return "1"
}

func (p *DdosUdpPortStats) getPath() string{
    return "ddos/udp-port/stats"
}

func (p *DdosUdpPortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosUdpPortStats,error) {
logger.Println("DdosUdpPortStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosUdpPortStats
    if err == nil {
        if len(axResp) > 0{
