...
type DdosDstEntrySrcPortStats20 struct {
	Inst struct {
		PortNum int `json:"port-num"`

		Protocol string `json:"protocol"`

		Stats DdosDstEntrySrcPortStats20Stats `json:"stats"`

		DstEntryName string
	} `json:"src-port"`
}

type DdosDstEntrySrcPortStats20Stats struct {
	TcpPort DdosDstEntrySrcPortStats20StatsTcpPort `json:"tcp-port"`
}

type DdosDstEntrySrcPortStats20StatsTcpPort struct {
...
func (p *DdosDstEntrySrcPortStats20) GetId() string {
	return "1"
}

func (p *DdosDstEntrySrcPortStats20) getPath() string {
	return "ddos/dst/entry/" + p.Inst.DstEntryName + "/src-port/" + strconv.Itoa(p.Inst.PortNum) + "+" + p.Inst.Protocol + "/stats?tcp-port=true"
}

func (p *DdosDstEntrySrcPortStats20) Post(authToken string, host string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstEntrySrcPortStats20::Post")
...
func (p *DdosDstEntrySrcPortStats20) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstEntrySrcPortStats20::Get")
	headers := axapi.GenRequestHeader(authToken)
...
func (p *DdosDstEntrySrcPortStats20) Put(authToken string, host string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstEntrySrcPortStats20::Put")
	headers := axapi.GenRequestHeader(authToken)
	payloadBytes, err := axapi.SerializeToJson(p)
	if err != nil {
		logger.Println("Failed to serialize struct as json", err)
...
func (p *DdosDstEntrySrcPortStats20) Delete(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstEntrySrcPortStats20::Delete")
	headers := axapi.GenRequestHeader(authToken)
