...
type SlbHttp2Oper struct {
	Oper SlbHttp2OperOper `json:"oper"`
}
type DataSlbHttp2Oper struct {
	DtSlbHttp2Oper SlbHttp2Oper `json:"http2"`
}

type SlbHttp2OperOper struct {
	Http2CpuList []SlbHttp2OperOperHttp2CpuList `json:"http2-cpu-list"`
	CpuCount     int                            `json:"cpu-count"`
}

type SlbHttp2OperOperHttp2CpuList struct {
	...
	Frame_flood_detected                      int `json:"frame_flood_detected"`
	Stream_cancel_flood_detected              int `json:"stream_cancel_flood_detected"`
}

func (p *SlbHttp2Oper) GetId() string {
	return "1"
}

func (p *SlbHttp2Oper) getPath() string {
	return "slb/http2/oper"
}

func (p *SlbHttp2Oper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbHttp2Oper, error) {
	logger.Println("SlbHttp2Oper::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataSlbHttp2Oper
	if err == nil {
		if len(axResp) > 0 {
