...
type SlbTemplateCacheStats struct {
    
    Name string `json:"name"`
    Stats SlbTemplateCacheStatsStats `json:"stats"`

}
type DataSlbTemplateCacheStats struct {
    DtSlbTemplateCacheStats SlbTemplateCacheStats `json:"cache"`
}


type SlbTemplateCacheStatsStats struct {
    Hits int `json:"hits"`
    Miss int `json:"miss"`
...
func (p *SlbTemplateCacheStats) GetId() string{
    return "1"
}

func (p *SlbTemplateCacheStats) getPath() string{
    
    return "slb/template/cache/"+p.Name+"/stats"
}

func (p *SlbTemplateCacheStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbTemplateCacheStats,error) {
logger.Println("SlbTemplateCacheStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbTemplateCacheStats
    if err == nil {
        if len(axResp) > 0{
