...
type DdosDstEntrySrcPortRangeStats18 struct {
	Inst struct {

    Protocol string `json:"protocol"`
    SrcPortRangeEnd int `json:"src-port-range-end"`
    SrcPortRangeStart int `json:"src-port-range-start"`
    Stats DdosDstEntrySrcPortRangeStats18Stats `json:"stats"`
    DstEntryName string 
...
type DdosDstEntrySrcPortRangeStats18Stats struct {
    UdpPort DdosDstEntrySrcPortRangeStats18StatsUdpPort `json:"udp-port"`
}


type DdosDstEntrySrcPortRangeStats18StatsUdpPort struct {
...
func (p *DdosDstEntrySrcPortRangeStats18) GetId() string{
    return "1"
}

func (p *DdosDstEntrySrcPortRangeStats18) getPath() string{
    return "ddos/dst/entry/" +p.Inst.DstEntryName + "/src-port-range/" +strconv.Itoa(p.Inst.SrcPortRangeStart)+"+" +strconv.Itoa(p.Inst.SrcPortRangeEnd)+"+"+p.Inst.Protocol+"/stats?udp-port=true"
}

func (p *DdosDstEntrySrcPortRangeStats18) Post(authToken string, host string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntrySrcPortRangeStats18::Post")
...
func (p *DdosDstEntrySrcPortRangeStats18) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntrySrcPortRangeStats18::Get")
    headers := axapi.GenRequestHeader(authToken)
...
func (p *DdosDstEntrySrcPortRangeStats18) Put(authToken string, host string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntrySrcPortRangeStats18::Put")
    headers := axapi.GenRequestHeader(authToken)
    payloadBytes, err := axapi.SerializeToJson(p)
    if err != nil {
        logger.Println("Failed to serialize struct as json", err)
...
func (p *DdosDstEntrySrcPortRangeStats18) Delete(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntrySrcPortRangeStats18::Delete")
    headers := axapi.GenRequestHeader(authToken)
