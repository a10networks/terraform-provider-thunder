...
func (p *DdosDstZonePortRangePortIndStats) getPath() string{
    
    return "ddos/dst/zone/" +p.ZoneName + "/port-range/" +p.PortRangeStart + "+" +p.PortRangeEnd + "+" +p.Protocol + "/port-ind/stats"
}

func (p *DdosDstZonePortRangePortIndStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDstZonePortRangePortIndStats,error) {
logger.Println("DdosDstZonePortRangePortIndStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosDstZonePortRangePortIndStats
    if err == nil {
        if len(axResp) > 0{
