...
type VisibilityPacketCaptureCaptureConfigStats struct {
    
    Name string `json:"name"`
    Stats VisibilityPacketCaptureCaptureConfigStatsStats `json:"stats"`

}
type DataVisibilityPacketCaptureCaptureConfigStats struct {
    DtVisibilityPacketCaptureCaptureConfigStats VisibilityPacketCaptureCaptureConfigStats `json:"capture-config"`
}


type VisibilityPacketCaptureCaptureConfigStatsStats struct {
...
func (p *VisibilityPacketCaptureCaptureConfigStats) GetId() string{
    return "1"
}

func (p *VisibilityPacketCaptureCaptureConfigStats) getPath() string{
    
    return "visibility/packet-capture/capture-config/"+p.Name+"/stats"
}

func (p *VisibilityPacketCaptureCaptureConfigStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataVisibilityPacketCaptureCaptureConfigStats,error) {
logger.Println("VisibilityPacketCaptureCaptureConfigStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataVisibilityPacketCaptureCaptureConfigStats
    if err == nil {
        if len(axResp) > 0{
