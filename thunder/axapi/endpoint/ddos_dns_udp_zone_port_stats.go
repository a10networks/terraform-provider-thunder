...
type DataDdosDnsUdpZonePortStats struct {
	DtDdosDnsUdpZonePortStats DdosDnsUdpZonePortStats `json:"dns-udp-zone-port"`
}

type DdosDnsUdpZonePortStatsStats struct {
...
func (p *DdosDnsUdpZonePortStats) getPath() string {
	return "ddos/dns-udp-zone-port/stats"
}

func (p *DdosDnsUdpZonePortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDnsUdpZonePortStats, error) {
	logger.Println("DdosDnsUdpZonePortStats::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataDdosDnsUdpZonePortStats
	if err == nil {
		if len(axResp) > 0 {
