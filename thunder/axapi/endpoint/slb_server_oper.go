...
type SlbServerr struct {
	DataSlbServer SlbServerOper `json:"server"`
}
...
func (p *SlbServerOper) getPath() string {
	return "slb/server/" + url.QueryEscape(p.Name) + "/oper"
}

func (p *SlbServerOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (SlbServerr, error) {
	logger.Println("SlbServerOper::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var slbServer SlbServerr
	if err == nil {
		err = json.Unmarshal(axResp, &slbServer)
		if err != nil {
			logger.Println("json.Unmarshal() failed with error", err)
		}
	}
	return slbServer, err
}
