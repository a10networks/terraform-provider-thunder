...
type SystemHardwaree struct {
	DataSystemHardware SystemHardwareOper `json:"hardware"`
}
...
func (p *SystemHardwareOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (SystemHardwaree, error) {
	logger.Println("SystemHardwareOper::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var sh SystemHardwaree
	if err == nil {
		err = json.Unmarshal(axResp, &sh)
		if err != nil {
