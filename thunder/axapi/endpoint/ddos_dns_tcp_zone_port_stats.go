...
type DdosDnsTcpZonePortStats struct {
    
    Stats DdosDnsTcpZonePortStatsStats `json:"stats"`

}
type DataDdosDnsTcpZonePortStats struct {
    DtDdosDnsTcpZonePortStats DdosDnsTcpZonePortStats `json:"dns-tcp-zone-port"`
}


type DdosDnsTcpZonePortStatsStats struct {
...
func (p *DdosDnsTcpZonePortStats) GetId() string{
    return "1"
}

func (p *DdosDnsTcpZonePortStats) getPath() string{
    return "ddos/dns-tcp-zone-port/stats"
}

func (p *DdosDnsTcpZonePortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDnsTcpZonePortStats,error) {
logger.Println("DdosDnsTcpZonePortStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosDnsTcpZonePortStats
    if err == nil {
        if len(axResp) > 0{
