...
type DdosTcpPortStats struct {
	Stats DdosTcpPortStatsStats `json:"stats"`
}
type DataDdosTcpPortStats struct {
	DtDdosTcpPortStats DdosTcpPortStats `json:"tcp-port"`
}

type DdosTcpPortStatsStats struct {
...
func (p *DdosTcpPortStats) GetId() string {
	return "1"
}

func (p *DdosTcpPortStats) getPath() string {
	return "ddos/tcp-port/stats"
}

func (p *DdosTcpPortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosTcpPortStats, error) {
	logger.Println("DdosTcpPortStats::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataDdosTcpPortStats
	if err == nil {
		if len(axResp) > 0 {
