...
type SlbSslErrorOper struct {
    
    Oper SlbSslErrorOperOper `json:"oper"`

}
type DataSlbSslErrorOper struct {
    DtSlbSslErrorOper SlbSslErrorOper `json:"ssl-error"`
}


type SlbSslErrorOperOper struct {
    ...
    BadProtocolVersionNumber int `json:"bad-protocol-version-number"`
    ...
    WrongNumberOfKeyBits int `json:"wrong-number-of-key-bits"`
    ...
    WrongVersionNumber int `json:"wrong-version-number"`
...
func (p *SlbSslErrorOper) GetId() string{
    return "1"
}

func (p *SlbSslErrorOper) getPath() string{
    return "slb/ssl-error/oper"
}

func (p *SlbSslErrorOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbSslErrorOper,error) {
logger.Println("SlbSslErrorOper::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbSslErrorOper
    if err == nil {
        if len(axResp) > 0{
