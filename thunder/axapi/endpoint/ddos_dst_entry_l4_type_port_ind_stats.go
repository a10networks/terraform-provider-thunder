...
type DdosDstEntryL4TypePortIndStats struct {
    
    Stats DdosDstEntryL4TypePortIndStatsStats `json:"stats"`
    Protocol string 
    DstEntryName string 

}
type DataDdosDstEntryL4TypePortIndStats struct {
    DtDdosDstEntryL4TypePortIndStats DdosDstEntryL4TypePortIndStats `json:"port-ind"`
}


type DdosDstEntryL4TypePortIndStatsStats struct {
...
func (p *DdosDstEntryL4TypePortIndStats) GetId() string{
    return "1"
}

func (p *DdosDstEntryL4TypePortIndStats) getPath() string{
    
    return "ddos/dst/entry/" +p.DstEntryName + "/l4-type/" +p.Protocol + "/port-ind/stats"
}

func (p *DdosDstEntryL4TypePortIndStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDstEntryL4TypePortIndStats,error) {
logger.Println("DdosDstEntryL4TypePortIndStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosDstEntryL4TypePortIndStats
    if err == nil {
        if len(axResp) > 0{
