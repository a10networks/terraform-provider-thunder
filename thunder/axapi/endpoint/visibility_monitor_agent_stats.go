...
type VisibilityMonitorAgentStats struct {
    
    AgentName string `json:"agent-name"`
    Stats VisibilityMonitorAgentStatsStats `json:"stats"`

}
type DataVisibilityMonitorAgentStats struct {
    DtVisibilityMonitorAgentStats VisibilityMonitorAgentStats `json:"agent"`
}


type VisibilityMonitorAgentStatsStats struct {
...
func (p *VisibilityMonitorAgentStats) GetId() string{
    return "1"
}

func (p *VisibilityMonitorAgentStats) getPath() string{
    
    return "visibility/monitor/agent/"+p.AgentName+"/stats"
}

func (p *VisibilityMonitorAgentStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataVisibilityMonitorAgentStats,error) {
logger.Println("VisibilityMonitorAgentStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataVisibilityMonitorAgentStats
    if err == nil {
        if len(axResp) > 0{
