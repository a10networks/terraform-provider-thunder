...
type FwServerPortStats struct {
	PortNumber int `json:"port-number"`

	Protocol string `json:"protocol"`

	Stats FwServerPortStatsStats `json:"stats"`

	Server_name string
}
type DataFwServerPortStats struct {
	DtFwServerPortStats FwServerPortStats `json:"port"`
}

type FwServerPortStatsStats struct {
...
func (p *FwServerPortStats) GetId() string {
	return "1"
}

func (p *FwServerPortStats) getPath() string {

	return "fw/server/" + p.Server_name + "/port/" + strconv.Itoa(p.PortNumber) + "+" + p.Protocol + "/stats"
}

func (p *FwServerPortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataFwServerPortStats, error) {
	logger.Println("FwServerPortStats::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataFwServerPortStats
	if err == nil {
		if len(axResp) > 0 {
