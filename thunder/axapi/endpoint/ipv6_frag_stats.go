...
type Ipv6FragStats struct {
    
    Stats Ipv6FragStatsStats `json:"stats"`

}
type DataIpv6FragStats struct {
    DtIpv6FragStats Ipv6FragStats `json:"frag"`
}


type Ipv6FragStatsStats struct {
...
func (p *Ipv6FragStats) GetId() string{
    return "1"
}

func (p *Ipv6FragStats) getPath() string{
    return "ipv6/frag/stats"
}

func (p *Ipv6FragStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataIpv6FragStats,error) {
logger.Println("Ipv6FragStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataIpv6FragStats
    if err == nil {
        if len(axResp) > 0{
