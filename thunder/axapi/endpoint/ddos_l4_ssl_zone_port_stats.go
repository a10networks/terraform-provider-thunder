...
type DdosL4SslZonePortStats struct {
    
    Stats DdosL4SslZonePortStatsStats `json:"stats"`

}
type DataDdosL4SslZonePortStats struct {
    DtDdosL4SslZonePortStats DdosL4SslZonePortStats `json:"l4-ssl-zone-port"`
}


type DdosL4SslZonePortStatsStats struct {
...
func (p *DdosL4SslZonePortStats) GetId() string{
    return "1"
}

func (p *DdosL4SslZonePortStats) getPath() string{
    return "ddos/l4-ssl-zone-port/stats"
}

func (p *DdosL4SslZonePortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosL4SslZonePortStats,error) {
logger.Println("DdosL4SslZonePortStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosL4SslZonePortStats
    if err == nil {
        if len(axResp) > 0{
