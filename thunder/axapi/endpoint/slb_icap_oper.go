...
type SlbIcapOper struct {
    
    Oper SlbIcapOperOper `json:"oper"`

}
type DataSlbIcapOper struct {
    DtSlbIcapOper SlbIcapOper `json:"icap"`
}


type SlbIcapOperOper struct {
    IcapCpuList []SlbIcapOperOperIcapCpuList `json:"icap-cpu-list"`
    CpuCount int `json:"cpu-count"`
}


type SlbIcapOperOperIcapCpuList struct {
...
func (p *SlbIcapOper) GetId() string{
    return "1"
}

func (p *SlbIcapOper) getPath() string{
    return "slb/icap/oper"
}

func (p *SlbIcapOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbIcapOper,error) {
logger.Println("SlbIcapOper::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbIcapOper
    if err == nil {
        if len(axResp) > 0{
