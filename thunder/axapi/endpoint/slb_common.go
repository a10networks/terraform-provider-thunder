...
type SlbCommon struct {
	Inst struct {
		...
		DnsCacheHitcountEnable int `json:"dns-cache-hitcount-enable"`
		...
		MaxLocalRate int `json:"max-local-rate" dval:"64"`
		...
		OddEvenNatOneArm int `json:"odd-even-nat-one-arm"`
		...
		RedirectDummyEthernet int `json:"redirect-dummy-ethernet"`

		RedirectDummyVlan int `json:"redirect-dummy-vlan"`
