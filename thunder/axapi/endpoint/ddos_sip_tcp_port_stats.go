...
type DdosSipTcpPortStats struct {
    
    Stats DdosSipTcpPortStatsStats `json:"stats"`

}
type DataDdosSipTcpPortStats struct {
    DtDdosSipTcpPortStats DdosSipTcpPortStats `json:"sip-tcp-port"`
}


type DdosSipTcpPortStatsStats struct {
...
func (p *DdosSipTcpPortStats) GetId() string{
    return "1"
}

func (p *DdosSipTcpPortStats) getPath() string{
    return "ddos/sip-tcp-port/stats"
}

func (p *DdosSipTcpPortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosSipTcpPortStats,error) {
logger.Println("DdosSipTcpPortStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosSipTcpPortStats
    if err == nil {
        if len(axResp) > 0{
