...
type DdosSrcEntryOper struct {
    
    Oper DdosSrcEntryOperOper `json:"oper"`
    SrcEntryName string `json:"src-entry-name"`

}
type DataDdosSrcEntryOper struct {
    DtDdosSrcEntryOper DdosSrcEntryOper `json:"entry"`
}


type DdosSrcEntryOperOper struct {
    Ddos_entry_list []DdosSrcEntryOperOperDdos_entry_list `json:"ddos_entry_list"`
...
type DdosSrcEntryOperOperDdos_entry_list struct {
...
func (p *DdosSrcEntryOper) GetId() string{
    return "1"
}

func (p *DdosSrcEntryOper) getPath() string{
    
    return "ddos/src/entry/"+p.SrcEntryName+"/oper"
}

func (p *DdosSrcEntryOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosSrcEntryOper,error) {
logger.Println("DdosSrcEntryOper::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosSrcEntryOper
    if err == nil {
        if len(axResp) > 0{
