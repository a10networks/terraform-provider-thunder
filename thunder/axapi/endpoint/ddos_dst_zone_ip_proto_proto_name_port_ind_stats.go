...
type DdosDstZoneIpProtoProtoNamePortIndStats struct {
    
    Stats DdosDstZoneIpProtoProtoNamePortIndStatsStats `json:"stats"`
    ZoneName string 
    Protocol string 

}
type DataDdosDstZoneIpProtoProtoNamePortIndStats struct {
    DtDdosDstZoneIpProtoProtoNamePortIndStats DdosDstZoneIpProtoProtoNamePortIndStats `json:"port-ind"`
}


type DdosDstZoneIpProtoProtoNamePortIndStatsStats struct {
...
func (p *DdosDstZoneIpProtoProtoNamePortIndStats) GetId() string{
    return "1"
}

func (p *DdosDstZoneIpProtoProtoNamePortIndStats) getPath() string{
    
    return "ddos/dst/zone/" +p.ZoneName + "/ip-proto/proto-name/" +p.Protocol + "/port-ind/stats"
}

func (p *DdosDstZoneIpProtoProtoNamePortIndStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDstZoneIpProtoProtoNamePortIndStats,error) {
logger.Println("DdosDstZoneIpProtoProtoNamePortIndStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosDstZoneIpProtoProtoNamePortIndStats
    if err == nil {
        if len(axResp) > 0{
