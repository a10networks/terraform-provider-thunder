...
type VpnIkeStatsGlobalStats struct {
    
    Stats VpnIkeStatsGlobalStatsStats `json:"stats"`

}
type DataVpnIkeStatsGlobalStats struct {
    DtVpnIkeStatsGlobalStats VpnIkeStatsGlobalStats `json:"ike-stats-global"`
}


type VpnIkeStatsGlobalStatsStats struct {
...
func (p *VpnIkeStatsGlobalStats) GetId() string{
    return "1"
}

func (p *VpnIkeStatsGlobalStats) getPath() string{
    return "vpn/ike-stats-global/stats"
}

func (p *VpnIkeStatsGlobalStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataVpnIkeStatsGlobalStats,error) {
logger.Println("VpnIkeStatsGlobalStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataVpnIkeStatsGlobalStats
    if err == nil {
        if len(axResp) > 0{
