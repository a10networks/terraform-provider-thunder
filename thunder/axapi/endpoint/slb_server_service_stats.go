...
type SlbServerServiceStats48 struct {
	...
	Stats SlbServerServiceStats48Stats `json:"stats"`

	Server_name string
}
type DataSlbServerServiceStats48 struct {
	DtSlbServerServiceStats48 SlbServerServiceStats48 `json:"service"`
}

type SlbServerServiceStats48Stats struct {
...
func (p *SlbServerServiceStats48) GetId() string {
	return "1"
}

func (p *SlbServerServiceStats48) getPath() string {

	return "slb/server/" + p.Server_name + "/service/" + strconv.Itoa(p.PortNumber) + "+" + p.Protocol + "+" + p.Label + "/stats"
}

func (p *SlbServerServiceStats48) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbServerServiceStats48, error) {
	logger.Println("SlbServerServiceStats48::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataSlbServerServiceStats48
	if err == nil {
		if len(axResp) > 0 {
