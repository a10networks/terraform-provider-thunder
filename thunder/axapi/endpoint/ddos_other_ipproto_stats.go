...
type DdosOtherIpprotoStats struct {
    
    Stats DdosOtherIpprotoStatsStats `json:"stats"`

}
type DataDdosOtherIpprotoStats struct {
    DtDdosOtherIpprotoStats DdosOtherIpprotoStats `json:"other-ipproto"`
}


type DdosOtherIpprotoStatsStats struct {
...
func (p *DdosOtherIpprotoStats) GetId() string{
    return "1"
}

func (p *DdosOtherIpprotoStats) getPath() string{
    return "ddos/other-ipproto/stats"
}

func (p *DdosOtherIpprotoStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosOtherIpprotoStats,error) {
logger.Println("DdosOtherIpprotoStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosOtherIpprotoStats
    if err == nil {
        if len(axResp) > 0{
