...
type SlbSmtpStats struct {
    
    Stats SlbSmtpStatsStats `json:"stats"`

}
type DataSlbSmtpStats struct {
    DtSlbSmtpStats SlbSmtpStats `json:"smtp"`
}


type SlbSmtpStatsStats struct {
...
func (p *SlbSmtpStats) GetId() string{
    return "1"
}

func (p *SlbSmtpStats) getPath() string{
    return "slb/smtp/stats"
}

func (p *SlbSmtpStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbSmtpStats,error) {
logger.Println("SlbSmtpStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbSmtpStats
    if err == nil {
        if len(axResp) > 0{
