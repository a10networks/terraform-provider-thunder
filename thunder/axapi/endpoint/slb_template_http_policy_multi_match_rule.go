...
type SlbTemplateHttpPolicyMultiMatchRule struct {
	Inst struct {
		...
		CookieNameValueMatch int `json:"cookie-name-value-match"`
		...
		HeaderNameValueMatch int `json:"header-name-value-match"`
...
func (p *SlbTemplateHttpPolicyMultiMatchRule) getPath() string {
	return "slb/template/http-policy/" + p.Inst.Http_policy_name + "/multi-match-rule"
}
