...
type DataSystemDnsCacheStats struct {
    DtSystemDnsCacheStats SystemDnsCacheStats `json:"dns-cache"`
}
...
func (p *SystemDnsCacheStats) getPath() string{
    return "system/dns-cache/stats"
}

func (p *SystemDnsCacheStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSystemDnsCacheStats,error) {
logger.Println("SystemDnsCacheStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSystemDnsCacheStats
    if err == nil {
        if len(axResp) > 0{
