...
type DataSessionsOper struct {
	DtSessionsOper SessionsOper `json:"sessions"`
}
...
type SessionsOperOper struct {
	...
	Filter_debug         string                        `json:"filter_debug"`
	...
	Uie                  string                        `json:"uie"`
	PersistIpv4          int                           `json:"persist-ipv4"`
	PersistType          string                        `json:"persist-type"`
	PersistSourceAddr    string                        `json:"persist-source-addr"`
	PersistSourcePort    int                           `json:"persist-source-port"`
	PersistDestAddr      string                        `json:"persist-dest-addr"`
...
type SessionsOperOperSessionList struct {
	...
	Radius_id               int                                              `json:"radius_id"`
	Fwd_pkt_hash            int                                              `json:"fwd_pkt_hash"`
	Rev_pkt_hash            int                                              `json:"rev_pkt_hash"`
}
...
func (p *SessionsOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSessionsOper, error) {
	logger.Println("SessionsOper::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataSessionsOper
	if err == nil {
		if len(axResp) > 0 {
