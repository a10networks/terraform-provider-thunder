...
type SlbIcapHttpStats struct {
    
    Stats SlbIcapHttpStatsStats `json:"stats"`

}
type DataSlbIcapHttpStats struct {
    DtSlbIcapHttpStats SlbIcapHttpStats `json:"icap_http"`
}


type SlbIcapHttpStatsStats struct {
...
func (p *SlbIcapHttpStats) GetId() string{
    return "1"
}

func (p *SlbIcapHttpStats) getPath() string{
    return "slb/icap_http/stats"
}

func (p *SlbIcapHttpStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbIcapHttpStats,error) {
logger.Println("SlbIcapHttpStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbIcapHttpStats
    if err == nil {
        if len(axResp) > 0{
