...
type DdosDstEntrySrcPortStats19 struct {
	Inst struct {
		PortNum int `json:"port-num"`

		Protocol string `json:"protocol"`

		Stats DdosDstEntrySrcPortStats19Stats `json:"stats"`

		DstEntryName string
	} `json:"src-port"`
}

type DdosDstEntrySrcPortStats19Stats struct {
	DnsUdpPort DdosDstEntrySrcPortStats19StatsDnsUdpPort `json:"dns-udp-port"`
}

type DdosDstEntrySrcPortStats19StatsDnsUdpPort struct {
...
func (p *DdosDstEntrySrcPortStats19) GetId() string {
	return "1"
}

func (p *DdosDstEntrySrcPortStats19) getPath() string {
	return "ddos/dst/entry/" + p.Inst.DstEntryName + "/src-port/" + strconv.Itoa(p.Inst.PortNum) + "+" + p.Inst.Protocol + "/stats?dns-udp-port=true"
}

func (p *DdosDstEntrySrcPortStats19) Post(authToken string, host string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstEntrySrcPortStats19::Post")
...
func (p *DdosDstEntrySrcPortStats19) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstEntrySrcPortStats19::Get")
	headers := axapi.GenRequestHeader(authToken)
...
func (p *DdosDstEntrySrcPortStats19) Put(authToken string, host string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstEntrySrcPortStats19::Put")
	headers := axapi.GenRequestHeader(authToken)
	payloadBytes, err := axapi.SerializeToJson(p)
	if err != nil {
		logger.Println("Failed to serialize struct as json", err)
...
func (p *DdosDstEntrySrcPortStats19) Delete(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstEntrySrcPortStats19::Delete")
	headers := axapi.GenRequestHeader(authToken)
