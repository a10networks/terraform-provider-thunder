...
type SlbSpdyProxyOper struct {
    
    Oper SlbSpdyProxyOperOper `json:"oper"`

}
type DataSlbSpdyProxyOper struct {
    DtSlbSpdyProxyOper SlbSpdyProxyOper `json:"spdy-proxy"`
}


type SlbSpdyProxyOperOper struct {
    L4CpuList []SlbSpdyProxyOperOperL4CpuList `json:"l4-cpu-list"`
    CpuCount int `json:"cpu-count"`
}


type SlbSpdyProxyOperOperL4CpuList struct {
...
func (p *SlbSpdyProxyOper) GetId() string{
    return "1"
}

func (p *SlbSpdyProxyOper) getPath() string{
    return "slb/spdy-proxy/oper"
}

func (p *SlbSpdyProxyOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbSpdyProxyOper,error) {
logger.Println("SlbSpdyProxyOper::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbSpdyProxyOper
    if err == nil {
        if len(axResp) > 0{
