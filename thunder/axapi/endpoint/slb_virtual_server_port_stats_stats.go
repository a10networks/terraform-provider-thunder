...
type SlbVirtualServerPortStats56Stats struct {
	Stats SlbVirtualServerPortStats56StatsStats `json:"stats"`
}

type SlbVirtualServerPortStats56StatsStats struct {
	ReqReceived            int `json:"req-received"`
	ReqForward             int `json:"req-forward"`
	ReqBlocked             int `json:"req-blocked"`
	ReqRedirected          int `json:"req-redirected"`
...
func (p *SlbVirtualServerPortStats56) getPath() string {
	...
	return "slb/virtual-server/" + p.Virtual_server_name + "/port/" + strconv.Itoa(p.PortNumber) + "+" + p.Protocol + "/stats?stats=true"
}
