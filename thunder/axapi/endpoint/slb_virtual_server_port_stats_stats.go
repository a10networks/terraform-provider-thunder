...
type DataSlbVirtualServerPortStats55 struct {
    DtSlbVirtualServerPortStats55 SlbVirtualServerPortStats55 `json:"port"`
}


type SlbVirtualServerPortStats55Stats struct {
    Stats SlbVirtualServerPortStats55StatsStats `json:"stats"`
}


type SlbVirtualServerPortStats55StatsStats struct {
...
func (p *SlbVirtualServerPortStats55) getPath() string{
    ...
    return "slb/virtual-server/" +p.Name + "/port/" +strconv.Itoa(p.PortNumber)+"+"+p.Protocol+"/stats?stats=true"
}

func (p *SlbVirtualServerPortStats55) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbVirtualServerPortStats55,error) {
logger.Println("SlbVirtualServerPortStats55::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbVirtualServerPortStats55
    if err == nil {
        if len(axResp) > 0{
