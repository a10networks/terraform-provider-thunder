...
type WebCategoryCategoryListStats struct {
    
    Name string `json:"name"`
    Stats WebCategoryCategoryListStatsStats `json:"stats"`

}
type DataWebCategoryCategoryListStats struct {
    DtWebCategoryCategoryListStats WebCategoryCategoryListStats `json:"category-list"`
}


type WebCategoryCategoryListStatsStats struct {
...
func (p *WebCategoryCategoryListStats) GetId() string{
    return "1"
}

func (p *WebCategoryCategoryListStats) getPath() string{
    
    return "web-category/category-list/"+p.Name+"/stats"
}

func (p *WebCategoryCategoryListStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataWebCategoryCategoryListStats,error) {
logger.Println("WebCategoryCategoryListStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataWebCategoryCategoryListStats
    if err == nil {
        if len(axResp) > 0{
