...
type DdosSipUdpPortStats struct {
    
    Stats DdosSipUdpPortStatsStats `json:"stats"`

}
type DataDdosSipUdpPortStats struct {
    DtDdosSipUdpPortStats DdosSipUdpPortStats `json:"sip-udp-port"`
}


type DdosSipUdpPortStatsStats struct {
...
func (p *DdosSipUdpPortStats) GetId() string{
    return "1"
}

func (p *DdosSipUdpPortStats) getPath() string{
    return "ddos/sip-udp-port/stats"
}

func (p *DdosSipUdpPortStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosSipUdpPortStats,error) {
logger.Println("DdosSipUdpPortStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosSipUdpPortStats
    if err == nil {
        if len(axResp) > 0{
