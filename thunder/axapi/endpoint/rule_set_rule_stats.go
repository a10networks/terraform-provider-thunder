...
type RuleSetRuleStatsStats struct {
	...
	SynCookieSynAckSent         int `json:"syn-cookie-syn-ack-sent"`
	SynCookieVerificationPassed int `json:"syn-cookie-verification-passed"`
	SynCookieVerificationFailed int `json:"syn-cookie-verification-failed"`
	TcpHalfOpenCount            int `json:"tcp-half-open-count"`
}
...
func (p *RuleSetRuleStats) getPath() string {
	...
	return "rule-set/" + p.Rule_set_name + "/rule/" + p.Name + "/stats"
}
