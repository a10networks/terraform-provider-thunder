...
type SlbServerStats struct {
	Name string `json:"name"`

	PortList []SlbServerStatsPortList `json:"port-list"`

	ServiceList []SlbServerStatsServiceList `json:"service-list"`

	Stats SlbServerStatsStats `json:"stats"`
}
type DataSlbServerStats struct {
	DtSlbServerStats SlbServerStats `json:"server"`
}

type SlbServerStatsPortList struct {
	PortNumber int                         `json:"port-number"`
	Protocol   string                      `json:"protocol"`
	Stats      SlbServerStatsPortListStats `json:"stats"`
}

type SlbServerStatsPortListStats struct {
...
type SlbServerStatsServiceList struct {
	PortNumber int                            `json:"port-number"`
	Protocol   string                         `json:"protocol"`
	Label      string                         `json:"label"`
	Stats      SlbServerStatsServiceListStats `json:"stats"`
}

type SlbServerStatsServiceListStats struct {
	Curr_conn                                 int `json:"curr_conn"`
	Curr_req                                  int `json:"curr_req"`
	Total_req                                 int `json:"total_req"`
	Total_req_succ                            int `json:"total_req_succ"`
...
type SlbServerStatsStats struct {
...
func (p *SlbServerStats) GetId() string {
	return "1"
}

func (p *SlbServerStats) getPath() string {
	return "slb/server/" + p.Name + "/stats"
}

func (p *SlbServerStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbServerStats, error) {
	logger.Println("SlbServerStats::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataSlbServerStats
	if err == nil {
		if len(axResp) > 0 {
