...
type DdosDstZonePortZoneServiceStats36 struct {
	Inst struct {
		PortNum int `json:"port-num"`

		Protocol string `json:"protocol"`

		Stats DdosDstZonePortZoneServiceStats36Stats `json:"stats"`

		ZoneName string
	} `json:"zone-service"`
}

type DdosDstZonePortZoneServiceStats36Stats struct {
	QuicZonePort DdosDstZonePortZoneServiceStats36StatsQuicZonePort `json:"quic-zone-port"`
}

type DdosDstZonePortZoneServiceStats36StatsQuicZonePort struct {
	...
	Retry_auth_pass                           int `json:"retry_auth_pass"`
	Retry_auth_fail                           int `json:"retry_auth_fail"`
	Retry_auth_sent                           int `json:"retry_auth_sent"`
	Connection_close_sent                     int `json:"connection_close_sent"`
	Invalid_retry_token                       int `json:"invalid_retry_token"`
	Short_header                              int `json:"short_header"`
	Short_header_drop                         int `json:"short_header_drop"`
	Encrypt_fail                              int `json:"encrypt_fail"`
	Decrypt_fail                              int `json:"decrypt_fail"`
	Encrypt_success                           int `json:"encrypt_success"`
	Decrypt_success                           int `json:"decrypt_success"`
	Rtt_drop0                                 int `json:"rtt_drop0"`
	Dynamic_count_warn                        int `json:"dynamic_count_warn"`
	Aead_pkt_rate_exceed                      int `json:"aead_pkt_rate_exceed"`
	Dcid_pkt_rate_exceed                      int `json:"dcid_pkt_rate_exceed"`
	Create_conn_init_only                     int `json:"create_conn_init_only"`
	Src_zone_service_entry_hit                int `json:"src_zone_service_entry_hit"`
	Same_sport_dport_drop                     int `json:"same_sport_dport_drop"`
	Version_no_match_drop                     int `json:"version_no_match_drop"`
	Src_ip_filtering_drop                     int `json:"src_ip_filtering_drop"`
	Src_ip_filtering_bypass                   int `json:"src_ip_filtering_bypass"`
}

func (p *DdosDstZonePortZoneServiceStats36) GetId() string {
	return "1"
}

func (p *DdosDstZonePortZoneServiceStats36) getPath() string {
	return "ddos/dst/zone/" + p.Inst.ZoneName + "/port/zone-service/" + strconv.Itoa(p.Inst.PortNum) + "+" + p.Inst.Protocol + "/stats?quic-zone-port=true"
}

func (p *DdosDstZonePortZoneServiceStats36) Post(authToken string, host string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstZonePortZoneServiceStats36::Post")
...
func (p *DdosDstZonePortZoneServiceStats36) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstZonePortZoneServiceStats36::Get")
	headers := axapi.GenRequestHeader(authToken)
...
func (p *DdosDstZonePortZoneServiceStats36) Put(authToken string, host string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstZonePortZoneServiceStats36::Put")
	headers := axapi.GenRequestHeader(authToken)
	payloadBytes, err := axapi.SerializeToJson(p)
	if err != nil {
		logger.Println("Failed to serialize struct as json", err)
...
func (p *DdosDstZonePortZoneServiceStats36) Delete(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
	logger.Println("DdosDstZonePortZoneServiceStats36::Delete")
	headers := axapi.GenRequestHeader(authToken)
