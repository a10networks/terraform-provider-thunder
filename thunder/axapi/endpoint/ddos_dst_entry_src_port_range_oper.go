...
type DdosDstEntrySrcPortRangeOper struct {
    
    Oper DdosDstEntrySrcPortRangeOperOper `json:"oper"`
...
type DataDdosDstEntrySrcPortRangeOper struct {
    DtDdosDstEntrySrcPortRangeOper DdosDstEntrySrcPortRangeOper `json:"src-port-range"`
}


type DdosDstEntrySrcPortRangeOperOper struct {
    Ddos_entry_list []DdosDstEntrySrcPortRangeOperOperDdos_entry_list `json:"ddos_entry_list"`
...
type DdosDstEntrySrcPortRangeOperOperDdos_entry_list struct {
...
func (p *DdosDstEntrySrcPortRangeOper) GetId() string{
    return "1"
}

func (p *DdosDstEntrySrcPortRangeOper) getPath() string{
    
    return "ddos/dst/entry/" +p.DstEntryName + "/src-port-range/" +strconv.Itoa(p.SrcPortRangeStart)+"+" +strconv.Itoa(p.SrcPortRangeEnd)+"+"+p.Protocol+"/oper"
}

func (p *DdosDstEntrySrcPortRangeOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDstEntrySrcPortRangeOper,error) {
logger.Println("DdosDstEntrySrcPortRangeOper::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosDstEntrySrcPortRangeOper
    if err == nil {
        if len(axResp) > 0{
