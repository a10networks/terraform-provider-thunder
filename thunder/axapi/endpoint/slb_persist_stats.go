...
type SlbPersistStats struct {
    
    Stats SlbPersistStatsStats `json:"stats"`

}
type DataSlbPersistStats struct {
    DtSlbPersistStats SlbPersistStats `json:"persist"`
}


type SlbPersistStatsStats struct {
...
func (p *SlbPersistStats) GetId() string{
    return "1"
}

func (p *SlbPersistStats) getPath() string{
    return "slb/persist/stats"
}

func (p *SlbPersistStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbPersistStats,error) {
logger.Println("SlbPersistStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbPersistStats
    if err == nil {
        if len(axResp) > 0{
