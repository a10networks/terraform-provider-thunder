...
type SlbDnsStats struct {
    
    Stats SlbDnsStatsStats `json:"stats"`

}
type DataSlbDnsStats struct {
    DtSlbDnsStats SlbDnsStats `json:"dns"`
}


type SlbDnsStatsStats struct {
...
func (p *SlbDnsStats) GetId() string{
    return "1"
}

func (p *SlbDnsStats) getPath() string{
    return "slb/dns/stats"
}

func (p *SlbDnsStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbDnsStats,error) {
logger.Println("SlbDnsStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbDnsStats
    if err == nil {
        if len(axResp) > 0{
