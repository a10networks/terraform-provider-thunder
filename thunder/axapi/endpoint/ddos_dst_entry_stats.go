...
type DdosDstEntryStats struct {
	DstEntryName string `json:"dst-entry-name"`

	Stats DdosDstEntryStatsStats `json:"stats"`
}
type DataDdosDstEntryStats struct {
	DtDdosDstEntryStats DdosDstEntryStats `json:"entry"`
}

type DdosDstEntryStatsStats struct {
...
func (p *DdosDstEntryStats) GetId() string {
	return "1"
}

func (p *DdosDstEntryStats) getPath() string {
	return "ddos/dst/entry/" + p.DstEntryName + "/stats"
}

func (p *DdosDstEntryStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDstEntryStats, error) {
	logger.Println("DdosDstEntryStats::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataDdosDstEntryStats
	if err == nil {
		if len(axResp) > 0 {
