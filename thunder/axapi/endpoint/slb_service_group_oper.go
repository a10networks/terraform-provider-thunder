...
type SlbServiceGroupOper struct {
    
    MemberList []SlbServiceGroupOperMemberList `json:"member-list"`
    Name string `json:"name"`
    Oper SlbServiceGroupOperOper `json:"oper"`

}
type DataSlbServiceGroupOper struct {
    DtSlbServiceGroupOper SlbServiceGroupOper `json:"service-group"`
}


type SlbServiceGroupOperMemberList struct {
    Name string `json:"name"`
    Port int `json:"port"`
    Oper SlbServiceGroupOperMemberListOper `json:"oper"`
}


type SlbServiceGroupOperMemberListOper struct {
    State string `json:"state"`
    HmKey int `json:"hm-key"`
    HmIndex int `json:"hm-index"`
    DrsList []SlbServiceGroupOperMemberListOperDrsList `json:"drs-list"`
    AltList []SlbServiceGroupOperMemberListOperAltList `json:"alt-list"`
}


type SlbServiceGroupOperMemberListOperDrsList struct {
...
type SlbServiceGroupOperMemberListOperAltList struct {
...
type SlbServiceGroupOperOper struct {
    State string `json:"state"`
    ...
    SgmList []SlbServiceGroupOperOperSgmList `json:"sgm-list"`
}


type SlbServiceGroupOperOperSgmList struct {
    SgmName string `json:"sgm-name"`
    SgmPort int `json:"sgm-port"`
}

func (p *SlbServiceGroupOper) GetId() string{
    return "1"
}

func (p *SlbServiceGroupOper) getPath() string{
    
    return "slb/service-group/"+p.Name+"/oper"
}

func (p *SlbServiceGroupOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbServiceGroupOper,error) {
logger.Println("SlbServiceGroupOper::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbServiceGroupOper
    if err == nil {
        if len(axResp) > 0{
