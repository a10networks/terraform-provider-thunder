...
type GslbZoneServiceStatsStats struct {
	...
	SmruleRedirFromSvcHit  int `json:"smrule-redir-from-svc-hit"`
	SmruleRedirFromSvcMiss int `json:"smrule-redir-from-svc-miss"`
}
...
func (p *GslbZoneServiceStats) getPath() string {
	...
	return "gslb/zone/" + p.Zone_name + "/service/" + strconv.Itoa(p.ServicePort) + "+" + p.ServiceName + "/stats"
}
