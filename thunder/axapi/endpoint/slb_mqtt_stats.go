...
type SlbMqttStats struct {
    
    Stats SlbMqttStatsStats `json:"stats"`

}
type DataSlbMqttStats struct {
    DtSlbMqttStats SlbMqttStats `json:"mqtt"`
}


type SlbMqttStatsStats struct {
...
func (p *SlbMqttStats) GetId() string{
    return "1"
}

func (p *SlbMqttStats) getPath() string{
    return "slb/mqtt/stats"
}

func (p *SlbMqttStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbMqttStats,error) {
logger.Println("SlbMqttStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataSlbMqttStats
    if err == nil {
        if len(axResp) > 0{
