...
type DdosDstEntryPortRangePortIndStats struct {
    
    Stats DdosDstEntryPortRangePortIndStatsStats `json:"stats"`
    Protocol string 
    PortRangeEnd string 
...
type DataDdosDstEntryPortRangePortIndStats struct {
    DtDdosDstEntryPortRangePortIndStats DdosDstEntryPortRangePortIndStats `json:"port-ind"`
}


type DdosDstEntryPortRangePortIndStatsStats struct {
...
func (p *DdosDstEntryPortRangePortIndStats) GetId() string{
    return "1"
}

func (p *DdosDstEntryPortRangePortIndStats) getPath() string{
    
    return "ddos/dst/entry/" +p.DstEntryName + "/port-range/" +p.PortRangeStart + "+" +p.PortRangeEnd + "+" +p.Protocol + "/port-ind/stats"
}

func (p *DdosDstEntryPortRangePortIndStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosDstEntryPortRangePortIndStats,error) {
logger.Println("DdosDstEntryPortRangePortIndStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataDdosDstEntryPortRangePortIndStats
    if err == nil {
        if len(axResp) > 0{
