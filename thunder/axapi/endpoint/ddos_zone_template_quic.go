...
type DdosZoneTemplateQuic struct {
	Inst struct {
		...
		DropOnNoVersionMatch int `json:"drop-on-no-version-match"`

		FixedBitCheckDisable int `json:"fixed-bit-check-disable"`

		PktRateCfg DdosZoneTemplateQuicPktRateCfg `json:"pkt-rate-cfg"`
...
type DdosZoneTemplateQuicPktRateCfg struct {
	QuicPktRateLimit int    `json:"quic-pkt-rate-limit"`
	QuicPktRate      int    `json:"quic-pkt-rate"`
	Per              string `json:"per"`
}
