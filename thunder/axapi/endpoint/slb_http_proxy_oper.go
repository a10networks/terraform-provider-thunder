...
type DataSlbHttpProxyOper struct {
	DtSlbHttpProxyOper SlbHttpProxyOper `json:"http-proxy"`
}
...
type SlbHttpProxyOperOperHttpProxyCpuList struct {
	...
	Http_policy_switching                int `json:"http_policy_switching"`
	...
	Http_policy_switching_ok             int `json:"http_policy_switching_ok"`
	...
	Http_policy_switching_enqueue        int `json:"http_policy_switching_enqueue"`
	...
	Req_http11_new_proxy                 int `json:"req_http11_new_proxy"`
	...
	Http_disallowed_methods              int `json:"http_disallowed_methods"`
	Http_allowed_methods                 int `json:"http_allowed_methods"`
	Upgrade_to_new_proxy                 int `json:"upgrade_to_new_proxy"`
	Del_h2c_header                       int `json:"del_h2c_header"`
...
func (p *SlbHttpProxyOper) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataSlbHttpProxyOper, error) {
	logger.Println("SlbHttpProxyOper::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataSlbHttpProxyOper
	if err == nil {
		if len(axResp) > 0 {
