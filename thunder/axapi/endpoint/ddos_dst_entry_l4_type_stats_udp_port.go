...
type DdosDstEntryL4TypeStats3 struct {
	Inst struct {

    Protocol string `json:"protocol"`
    Stats DdosDstEntryL4TypeStats3Stats `json:"stats"`
    DstEntryName string 
...
type DdosDstEntryL4TypeStats3Stats struct {
    UdpPort DdosDstEntryL4TypeStats3StatsUdpPort `json:"udp-port"`
}


type DdosDstEntryL4TypeStats3StatsUdpPort struct {
...
func (p *DdosDstEntryL4TypeStats3) GetId() string{
    return "1"
}

func (p *DdosDstEntryL4TypeStats3) getPath() string{
    return "ddos/dst/entry/" +p.Inst.DstEntryName + "/l4-type/"+p.Inst.Protocol+"/stats?udp-port=true"
}

func (p *DdosDstEntryL4TypeStats3) Post(authToken string, host string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntryL4TypeStats3::Post")
...
func (p *DdosDstEntryL4TypeStats3) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntryL4TypeStats3::Get")
    headers := axapi.GenRequestHeader(authToken)
...
func (p *DdosDstEntryL4TypeStats3) Put(authToken string, host string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntryL4TypeStats3::Put")
    headers := axapi.GenRequestHeader(authToken)
    payloadBytes, err := axapi.SerializeToJson(p)
    if err != nil {
        logger.Println("Failed to serialize struct as json", err)
...
func (p *DdosDstEntryL4TypeStats3) Delete(authToken string, host string, instId string, logger *axapi.ThunderLog) error {
    logger.Println("DdosDstEntryL4TypeStats3::Delete")
    headers := axapi.GenRequestHeader(authToken)
