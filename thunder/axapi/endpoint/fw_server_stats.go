...
type FwServerStats struct {
    
    Name string `json:"name"`
    PortList []FwServerStatsPortList `json:"port-list"`
    Stats FwServerStatsStats `json:"stats"`

}
type DataFwServerStats struct {
    DtFwServerStats FwServerStats `json:"server"`
}


type FwServerStatsPortList struct {
    PortNumber int `json:"port-number"`
    Protocol string `json:"protocol"`
    Stats FwServerStatsPortListStats `json:"stats"`
}


type FwServerStatsPortListStats struct {
...
type FwServerStatsStats struct {
...
func (p *FwServerStats) GetId() string{
    return "1"
}

func (p *FwServerStats) getPath() string{
    
    return "fw/server/"+p.Name+"/stats"
}

func (p *FwServerStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataFwServerStats,error) {
logger.Println("FwServerStats::Get")
    headers := axapi.GenRequestHeader(authToken)
    _, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
    var payload DataFwServerStats
    if err == nil {
        if len(axResp) > 0{
