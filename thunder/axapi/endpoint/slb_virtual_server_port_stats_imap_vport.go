...
type SlbVirtualServerPortStats52Stats struct {
	Imap_vport SlbVirtualServerPortStats52StatsImap_vport `json:"imap_vport"`
}

type SlbVirtualServerPortStats52StatsImap_vport struct {
	Num                      int `json:"num"`
	Curr                     int `json:"curr"`
...
func (p *SlbVirtualServerPortStats52) getPath() string {
	return "slb/virtual-server/" + p.Inst.Virtual_server_name + "/port/" + strconv.Itoa(p.Inst.PortNumber) + "+" + p.Inst.Protocol + "/stats?imap_vport=true"
}
