...
type DdosIpprotoIcmpStats struct {
	Stats DdosIpprotoIcmpStatsStats `json:"stats"`
}
type DataDdosIpprotoIcmpStats struct {
	DtDdosIpprotoIcmpStats DdosIpprotoIcmpStats `json:"ipproto-icmp"`
}

type DdosIpprotoIcmpStatsStats struct {
...
func (p *DdosIpprotoIcmpStats) GetId() string {
	return "1"
}

func (p *DdosIpprotoIcmpStats) getPath() string {
	return "ddos/ipproto-icmp/stats"
}

func (p *DdosIpprotoIcmpStats) Get(authToken string, host string, instId string, logger *axapi.ThunderLog) (DataDdosIpprotoIcmpStats, error) {
	logger.Println("DdosIpprotoIcmpStats::Get")
	headers := axapi.GenRequestHeader(authToken)
	_, axResp, err := axapi.SendGet(host, p.getPath(), "", nil, headers, logger)
	var payload DataDdosIpprotoIcmpStats
	if err == nil {
		if len(axResp) > 0 {
