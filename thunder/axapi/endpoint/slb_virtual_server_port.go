...
type SlbVirtualServerPort struct {
	Inst struct {
		...
		FastDnsCache string `json:"fast-dns-cache"`
		...
		WhenServerSelectionFailed string `json:"when-server-selection-failed"`
...
func (p *SlbVirtualServerPort) getPath() string {
	return "slb/virtual-server/" + p.Inst.Virtual_server_name + "/port"
}
