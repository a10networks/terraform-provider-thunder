...
- `ip_version` (String) 'ipv4': take action for IPv4 traffic-only; 'ipv6': take action for IPv6 traffic-only;
...
- `encap` (Block List, Max: 1) (see [below for nested schema](#nestedblock--local_device--traffic_redirection--encap))
...
<a id="nestedblock--local_device--traffic_redirection--encap"></a>
### Nested Schema for `local_device.traffic_redirection.encap`
...
- `default_user_group_count` (Number) Number of default traffic buckets
- `enable` (Number)
...
- `user_group_count` (Number) Number of traffic buckets
- `user_tag` (String) Customized tag
- `uuid` (String) uuid of the object
