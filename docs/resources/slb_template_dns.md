...
- `cache_hitcount_enable` (Number) Enable DNS cache entry hit count
...
- `category_lookup_bypass` (String) DNS type class-lists for bypassing category lookup
- `category_lookup_list` (Block List) (see [below for nested schema](#nestedblock--category_lookup_list))
- `category_lookup_online_lookup` (Number) Enable online webroot lookup
...
- `label_count_filter` (Block List, Max: 1) (see [below for nested schema](#nestedblock--label_count_filter))
- `label_length_filter` (Block List, Max: 1) (see [below for nested schema](#nestedblock--label_length_filter))
...
- `qps_log_low` (Number) low threshold for QPS logging (queries per second)
- `qps_threshold_log` (Number) enable threshold log for DNS QPS
...
- `tld_filter_log_enable` (Number) Enable dns tld filter logging
- `tld_filter_white_list` (String) white-list class-list name (string-insensitive type)
...
<a id="nestedblock--category_lookup_list"></a>
### Nested Schema for `category_lookup_list`
...
<a id="nestedblock--label_count_filter"></a>
### Nested Schema for `label_count_filter`
...
<a id="nestedblock--label_length_filter"></a>
### Nested Schema for `label_length_filter`

Optional:

- `drop_log_enable` (Number) enable the log when hit the rule
- `fqdn_label_length` (Block List) (see [below for nested schema](#nestedblock--label_length_filter--fqdn_label_length))
- `label_length_filter_action` (String) 'drop': drop; 'ignore': ignore;
- `uuid` (String) uuid of the object

<a id="nestedblock--label_length_filter--fqdn_label_length"></a>
### Nested Schema for `label_length_filter.fqdn_label_length`
...
- `cache_non_valid` (Number) Enable caching non-valid negative response, otherwise will only cache valid negative response
...
- `parallel_queries` (Number) Number of parallel queries to send to servers
...
- `nx_response_rate` (Number) Queries from entries whose NX Responses exceeding this rate within the window will be dropped (default 5 per second)
...
- `lid_nx_response_rate` (Number) Queries from entries whose NX Responses exceeding this rate within the window will be dropped (default 5 per second)
- `lid_response_rate` (Number) Responses exceeding this rate within the window will be dropped (default 5 per second), 0 for unlimited
