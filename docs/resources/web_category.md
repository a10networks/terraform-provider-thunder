...
- `cloud_query_cache_size` (Number) Maximum cache size for storing cloud query results, default: 1
...
- `dns_over_https` (Number) Category DNS over HTTPs
...
- `generative_ai` (Number) Category Generative AI
...
- `illegal_pornography` (Number) Category Illegal Pornography eg. Child Sexual Abuse
...
- `low_thc_cannabis_products` (Number) Category Low-THC Cannabis Products
...
- `nudity_artistic` (Number) Category Artistic Nudity
...
- `self_harm` (Number) Category Self Harm
