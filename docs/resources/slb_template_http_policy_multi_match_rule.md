...
- `http_policy_name` (String) Http_policy_name
- `multi_match` (String) Specify a multi-match-rule name

### Optional

- `cookie_name_contains_string` (String) Cookie name string
...
- `cookie_name_value_match` (Number) When enabled, the rule will match only when both cookie-name and cookie-value matches with same cookie
...
- `header_name_value_match` (Number) When enabled, the rule will match only when both header-name and header-value matches with same header
