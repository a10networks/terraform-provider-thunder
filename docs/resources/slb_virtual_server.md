...
- `shared_partition_vs_template` (Number) Reference a virtual-server template from shared partition
...
- `fast_dns_cache` (String) 'force-enable': Always enable; 'force-disable': Always disable; 'depends-on-config': Depends on configurations;
...
- `when_server_selection_failed` (String) 'send-504': Stay functional up and response with HTTP 504;
