...
- `domain_name` (String) Specify the domain name to be added in the domain list.
- `fail_interval` (Number) Set up the failure interval in second for the DNS resolution. (default is 30-second)
- `interval` (Number) Set up the query interval in minute for the DNS resolution. (default is 10-minute)
