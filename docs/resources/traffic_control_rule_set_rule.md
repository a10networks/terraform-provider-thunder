...
- `derived_attribute` (String) 'usergroup': Match the value from the derived attribute of user group in the class-list.; 'userid': Match the value from the derived attribute of user ID in the class-list.;
...
- `ip_version` (String) 'v4': IPv4 rule; 'v6': IPv6 rule; 'any': IP version is not specified. Only compatible with filters by application, zone or the source class-list of radius type.;
...
- `src_class_list_type` (String) 'radius': Match the value of specified RADIUS attribute in the class-list.;
