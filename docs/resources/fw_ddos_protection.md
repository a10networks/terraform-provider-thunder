...
- `action_type` (String) 'drop': Log, and drop all packets (default); 'redistribute-route': Log, Notify upstream router to reroute the packets. Drop all packets by default.;
...
- `forward` (Number) Forward traffic with rate-limiting
