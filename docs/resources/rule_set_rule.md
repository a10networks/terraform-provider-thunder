...
- `deny_reset_limit_policy` (Number) Limit policy Template (only works for inbound rule)
...
- `on_timeout` (Number) on-timeout for Syn-cookie (Timeout in seconds, default is 120 seconds (2 minutes))
...
- `skip_urpf_check` (Number) Skip Unicast Reverse Path Forwarding check
- `syn_cookie` (Number) Configure Firewall rule Syn-Cookie Protection
- `syn_cookie_enable` (String) 'enable': enable; 'disable': disable;
- `tcp` (Number) Firewall rule TCP parameters
- `threshold_val` (Number) Decimal number
