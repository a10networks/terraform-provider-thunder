...
- `dplane` (String) 'enable': Enable health-check on dplane; 'disable': Disable health-check on dplane; 'auto': Auto select(default);
...
- `ext_root_shell` (Number) Enable root-shell environment
...
- `radius_message_authenticator` (Number) message-authenticator type
