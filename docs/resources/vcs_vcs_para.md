...
- `hold_preemption_interval` (Number) The VCS Node will hold specified time interval before it start to challenge peer(s) for Mastership (in unit of second (default is 60))
- `link_poll_timeout` (Number)
- `memory_stat_interval` (Number) Interval of aVCS memory statistics record (minutes)
- `multicast_ip` (String) Multicast (group) IP address (Multicast IP address (224.0.0.211 by default))
...
- `ssl_cert` (String) Specify the cert file name
- `ssl_config` (Number) Configure SSL
- `ssl_enable` (Number) Enable SSL
- `ssl_key` (String) Specify the key file name
