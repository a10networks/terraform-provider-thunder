...
- `ddos_source_id_export` (Number) Enable exporting DDoS sourceID in sflow packet sample
- `default_counter_polling_mtu` (Number) Default MTU for counter-polling packets - DDoS 3.2 format only (Default: 1500)
- `enlarge_zone_name` (Number) Allow TPS to packet up to 127 character zone names
...
- `one_blk_per_pkt` (Number) Forces sFlow packet to only contain one sFlow counter block
...
- `sampling_rate_limit` (Number) sFlow packet sampling rate limit, default is 1000
