...
- `l2_fwd_for_me_arp_ns` (Number) Enable L2 forwarding for both ARP and NS packet with target IP/IPv6 addresses that are owned by the device
...
- `counters1` (String) 'all': all; 'xparent_vlan_list_err': Transparent Mode VLAN List Errors; 'asymmetric_route_drop_err': asymmetric_route_drop_err;
