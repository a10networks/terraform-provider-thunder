...
- `file_size` (Number) merged pcap file size limit (unit MB)
...
- `inner_ip` (Number) IP in IP or IP in IPv6 packets
- `inner_ipv4_address` (String) IP address
- `inner_ipv4_netmask` (String) IP subnet mask
- `inner_ipv6` (Number) IPv6 in IPv6 or IPv6 in IPv4 packets
- `inner_ipv6_address` (String) IPv6 address
...
- `l3_proto` (String) 'arp': arp; 'ip': ip; 'ipv6': ipv6; 'neighbor': neighbor;
